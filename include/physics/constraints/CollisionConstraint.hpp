...
namespace physics {
    ...
    inline constexpr float kCollisionMassEpsilon = 1e-8f;
    inline constexpr float kCollisionPenetrationSlop = 1.0e-4f;
    inline constexpr float kNormalCorrectionRelax = 0.92f;
    inline constexpr float kTangentCorrectionRelax = 0.97f;

    struct CollisionConstraint {
        CollisionConstraint() = default;

        CollisionConstraint(uint32_t a, uint32_t b, glm::vec3 normal, glm::vec3 offsetA,
                            glm::vec3 offsetB, float comp = 0.0f,
                            glm::vec3 warmMid = glm::vec3(0.0f), float warmLambda = 0.0f,
                            float frictionCoeff = kRigidContactStaticFrictionMu)
            : indexA(a), indexB(b), contactNormal(normal), localOffsetA(offsetA),
              localOffsetB(offsetB), warmSortMid(warmMid), frictionCoefficient(frictionCoeff),
              compliance(comp), warmStartLambda(warmLambda) {
        }

        void resetLambda() {
            lambda = warmStartLambda;
        }
        float accumulatedLambda() const {
            return lambda;
        }

        void solve(std::vector<physics::Vertex>& vertices, float deltatime) {
            ...
            if (w1 + w2 <= kCollisionMassEpsilon)
                return;
            ...
            if (C >= -kCollisionPenetrationSlop)
                return;
            ...
            if (denom <= kCollisionMassEpsilon)
                return;
            ...
            va.orientation =
                glm::normalize(va.orientation + 0.5f * glm::quat(0.0f, dOmegaA) * va.orientation);
            vb.orientation =
                glm::normalize(vb.orientation + 0.5f * glm::quat(0.0f, dOmegaB) * vb.orientation);
            ...
            const glm::vec3 tangentialSlide =
                relativeSlide - glm::dot(relativeSlide, contactNormal) * contactNormal;
            ...
            if (tangentInvMass <= kCollisionMassEpsilon)
                return;
            ...
            const glm::vec3 dOmegaTangentA =
                worldInvInertiaA * glm::cross(postOffsetA, tangentImpulse);
            const glm::vec3 dOmegaTangentB =
                worldInvInertiaB * glm::cross(postOffsetB, -tangentImpulse);
            va.orientation = glm::normalize(
                va.orientation + 0.5f * glm::quat(0.0f, dOmegaTangentA) * va.orientation);
            vb.orientation = glm::normalize(
                vb.orientation + 0.5f * glm::quat(0.0f, dOmegaTangentB) * vb.orientation);
        }
      ...
      private:
        ...
        float computeGeneralizedInverseMass(const physics::Vertex& v, const glm::vec3& worldOffset,
                                            const glm::vec3& direction,
                                            const glm::mat3& worldInvInertia) const {
            const glm::vec3 angular = glm::cross(worldOffset, direction);
            return v.invMass +
                   glm::dot(glm::cross(worldInvInertia * angular, worldOffset), direction);
        }
    };
