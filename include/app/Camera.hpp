...
namespace scrap {
     ...
     * Camera constructor
     */
        Camera(const scrap::CameraCreateInfo& createInfo)
            : pos(createInfo.pos), worldUp(createInfo.worldUp), yaw(createInfo.yaw),
              pitch(createInfo.pitch), fov(createInfo.fov), movementSpeed(createInfo.movementSpeed),
              mouseSensitivity(createInfo.mouseSensitivity), scrWidth(createInfo.scrWidth),
              scrHeight(createInfo.scrHeight) {
            updateView();
        }
   ...
   */
	 ...
	 */
        void processMouseMovement(float xoffset, float yoffset, bool constrainPitch = true) {
            ...
            if (constrainPitch) {
                pitch = std::max(std::min(pitch, CameraCreateInfo::PITCH_MAX),
                                 CameraCreateInfo::PITCH_MIN);
            }
