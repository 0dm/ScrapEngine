...
namespace scrap::ui {
    ...
    template <int N = 1> class DragInt : public ImGuiComponent {
      ...
      public:
        ...
        DragInt(const std::string& name, const std::string& label, std::array<int, N>& value,
                float speed = 1.0f, int min = 0, int max = 0, const std::string& format = "%d",
        ...
        void render() override {
            ...
            if constexpr (N == 1) {
                changed =
                    ImGui::DragInt(label.c_str(), value.data(), speed, min, max, format.c_str());
            } else if constexpr (N == 2) {
                changed =
                    ImGui::DragInt2(label.c_str(), value.data(), speed, min, max, format.c_str());
            } else if constexpr (N == 3) {
                changed =
                    ImGui::DragInt3(label.c_str(), value.data(), speed, min, max, format.c_str());
            } else if constexpr (N == 4) {
                changed =
                    ImGui::DragInt4(label.c_str(), value.data(), speed, min, max, format.c_str());
            }
