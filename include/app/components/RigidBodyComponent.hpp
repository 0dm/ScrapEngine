...
namespace scrap {
    ...
    class RigidBodyComponent : public Component {
      public:
        RigidBodyComponent(glm::vec3 initPosition, glm::vec3 initVelocity,
                           glm::quat initOrientation, glm::vec3 initAngularVelocity,
                           glm::vec3 accumulatedForce = glm::vec3(0.0f, 0.0f, 0.0f),
                           float invMass = 1.0f, glm::mat3 invInertiaTensor = glm::mat3(1.0f),
                           glm::vec3 initScale = glm::vec3(1.0f))
            : position(initPosition), velocity(initVelocity), orientation(initOrientation),
              angularVelocity(initAngularVelocity), accumulatedForce(accumulatedForce),
              invMass(invMass), invInertiaTensor(invInertiaTensor), storedInvMass(invMass),
              storedInvInertiaTensor(invInertiaTensor), scale(initScale) {
        }

        bool isDynamic() const {
            return invMass > 1e-8f;
        }
        bool canBeDynamic() const {
            return storedInvMass > 1e-8f;
        }
        ...
        glm::vec3 getWorldCenterOfMass() const {
            return position + orientation * (centerOfMass * scale);
        }
        ...
        const glm::mat3& getInvInertiaTensor() const {
        ...
        void setCenterOfMass(const glm::vec3& p) {
            centerOfMass = p;
        }
        void setScale(const glm::vec3& s) {
            scale = s;
        }
        void setWorldCenterOfMass(const glm::vec3& p) {
            position = p - orientation * (centerOfMass * scale);
        }
        ...
        void setAccumulatedForce(const glm::vec3& force) {
            accumulatedForce = force;
        }
        void setCollisionEnabled(bool enabled) {
            collisionEnabled = enabled;
        }
        void setInvMass(float w) {
            invMass = w;
        }
        void setInvInertiaTensor(const glm::mat3& I) {
            invInertiaTensor = I;
        }
        ...
        static glm::mat3 meshInvInertiaTensor(std::shared_ptr<modeling::Mesh> m,
                                              const glm::vec3& centerOfMass, float invMass);
        static glm::mat3 meshInvInertiaTensor(std::shared_ptr<modeling::Mesh> m,
                                              const glm::vec3& centerOfMass, float invMass,
