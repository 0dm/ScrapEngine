...
namespace scrap {
    ...
    class ClothComponent : public Component {
      public:
        ...
        const std::shared_ptr<modeling::Mesh>& getSourceMesh() const {
            return sourceMesh;
        }
        const std::shared_ptr<modeling::Mesh>& getRuntimeMesh() const {
            return runtimeMesh;
        }
        bool hasClothData() const {
            return clothData.has_value();
        }
        const std::string& getLastBuildError() const {
            return lastBuildError;
        }
        const ClothSettings& getSettings() const {
            return settings;
        }
        ...
        void markRuntimeMeshDirty() {
            runtimeMeshDirty = true;
        }
        bool isRuntimeMeshDirty() const {
            return runtimeMeshDirty;
        }
