...
namespace {
    ...
    using scrap::Entity;
    using scrap::MeshRendererComponent;
    using scrap::RigidBodyComponent;
    using scrap::Scene;
    using scrap::TransformComponent;
    ...
    std::optional<BoxShape> detectBoxShape(const scrap::modeling::Mesh& mesh) {
    ...
    std::optional<PlaneShape> detectPlaneShape(const scrap::modeling::Mesh& mesh) {
        const auto& vertices = mesh.getVertices();
    ...
    std::optional<float> computeOBBOverlapDepth(const OBB& a, const OBB& b) {
        ...
        const glm::vec3 t(glm::dot(tWorld, a.axes[0]), glm::dot(tWorld, a.axes[1]),
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            const float rb = b.halfExtents.x * absR[i][0] + b.halfExtents.y * absR[i][1] +
        ...
        for (int j = 0; j < 3; ++j) {
            const float ra = a.halfExtents.x * absR[0][j] + a.halfExtents.y * absR[1][j] +
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                ...
                const float distance = std::abs(t[(i + 2) % 3] * R[(i + 1) % 3][j] -
                                                t[(i + 1) % 3] * R[(i + 2) % 3][j]);
                if (!testAxis(distance, ra, rb)) {
                    return std::nullopt;
                }
    ...
    void collectDiagnostics(Scene& scene, OverlapStats& overlapStats,
                            PlanePenetrationStats& planeStats, float& maxSpeed) {
    ...
    RigidBodyComponent* findRigidBodyByName(const std::vector<RigidBodyComponent*>& rigidBodies,
    ...
    std::vector<PiecewiseTransferBlock> buildPiecewiseTransferPlan(
        ...
        for (int targetLayer = 0; targetLayer < 3; ++targetLayer) {
            ...
            for (int slot : kSlotOrder) {
                ...
                auto* rigidBody =
                    findRigidBodyByName(rigidBodies, std::format("JengaBlock_{}", sourceIndex));
                ...
                if (!rigidBody || sourcePosIt == initialPositions.end() ||
                    targetPosIt == initialPositions.end() ||
                    targetOriIt == initialOrientations.end()) {
                    continue;
                }
    ...
    bool advancePiecewiseTransferIfReady(PiecewiseTransferProgress& progress,
        ...
        if (progress.phase < 4) {
            ...
            const bool reached = hoverTimeout || (distance <= phaseDistanceThreshold &&
    ...
    void drivePiecewiseTransferBlock(physics::XPBDSolver& solver,
                                     const PiecewiseTransferBlock& block, int phase,
                                     float physicsDt) {
        auto* rigidBody = block.rigidBody;
        if (!rigidBody) {
        ...
        const float phaseScale = phase == 0 ? 1.20f : (phase == 3 ? 0.72f : 0.90f);
        ...
        const glm::vec3 clampedDesiredVelocity =
            ...
            clampVectorMagnitude(desiredVerticalVelocity,
        ...
        const glm::vec3 driveVelocity =
            ...
            (clampVectorMagnitude(desiredLateralAcceleration, lateralAccelerationLimit) +
             clampVectorMagnitude(desiredVerticalAcceleration, verticalAcceleration > 0.0f
                                                                   ? downwardAccelerationLimit
                                                                   : upwardAccelerationLimit)) *
                physicsDt;
    ...
    std::vector<ContactDumpEntry> collectContactDump(
        physics::XPBDSolver& solver, const std::vector<RigidBodyComponent*>& rigidBodies) {
        ...
        std::sort(
            dump.begin(), dump.end(),
            [](const ContactDumpEntry& a, const ContactDumpEntry& b) { return a.bodyA < b.bodyA; });
        return dump;
    }
...
namespace {
    ...
    std::string resolveSceneFilePath(const std::string& path) {
        ...
        const fs::path root = scrap::platform::findProjectRootNearExecutable();
        if (!root.empty()) {
...
int main(int argc, char** argv) {
    ...
    if (testStackStability) {
        ...
        removeBodyName.clear();
        std::cout
            << "[--test-piecewise-transfer] scripted drag rebuild of a second three-layer tower\n";
    } else {
        if (argc >= 2) {
            scenePath = argv[1];
        }
    ...
    scrap::CameraCreateInfo cameraInfo{
        .scrWidth = 1280.0f,
        .scrHeight = 720.0f,
    };
    ...
    scrap::physics_demo::armScene(scene);
    auto rigidBodies = scrap::physics_demo::collectRigidBodies(scene);

    physics::XPBDSolver solver;
    const auto tuning = scrap::physics_demo::selectRigidSolverTuning(dynamicBodyCount(rigidBodies));
    ...
    std::cout << "scene=" << scenePath << " bodies=" << rigidBodies.size()
              << " dynamic=" << dynamicBodyCount(rigidBodies)
              << " iterations=" << solver.solverIterations << " substeps=" << solver.rigidSubsteps
              << " dt=" << tuning.physicsDt << " wake_step=" << wakeStep
    ...
    for (int step = 0; step <= steps; ++step) {
        ...
        if (testPiecewiseTransfer) {
            ...
            transferProgress.peakOverlap =
                std::max(transferProgress.peakOverlap, overlapStats.maxDepth);
        ...
        if (!testStackStability && !testPiecewiseTransfer &&
            ...
            std::cout << "step=" << step << " max_speed=" << maxSpeed
                      << " box_overlaps=" << overlapStats.pairCount
                      << " max_box_overlap=" << overlapStats.maxDepth << " worst_pair="
                      << (overlapStats.bodyA.empty()
                              ? "-"
                              : overlapStats.bodyA + "/" + overlapStats.bodyB)
                      << " plane_penetrations=" << planeStats.pairCount
                      << " max_plane_penetration=" << planeStats.maxDepth << " worst_plane="
                      << (planeStats.body.empty() ? "-" : planeStats.body + "/" + planeStats.plane)
                      << "\n";
        }
        ...
        if (step == dumpStep) {
            ...
            for (size_t i = 0; i < std::min<size_t>(dump.size(), 12); ++i) {
                ...
                std::cout << "  [" << i << "] " << entry.bodyA << "/" << entry.bodyB << " normal=("
        ...
        scrap::physics_demo::applyForces(scene);
        if (testPiecewiseTransfer && transferProgress.activeIndex < transferPlan.size()) {
            if (transferProgress.phase < 4) {
                drivePiecewiseTransferBlock(solver, transferPlan[transferProgress.activeIndex],
                                            transferProgress.phase, tuning.physicsDt);
            } else {
                solver.dragBody = nullptr;
            }
        } else {
        ...
        scrap::physics_demo::syncRigidBodiesToTransforms(scene);
    ...
    if (testPiecewiseTransfer) {
        ...
        for (const auto& block : transferPlan) {
            ...
            maxFinalPlacementError =
                std::max(maxFinalPlacementError,
                         glm::distance(block.rigidBody->getPosition(), block.targetPosition));
        }

        std::cout << "piecewise_transfer_completed_blocks=" << transferProgress.completedBlocks
                  << " active_index=" << transferProgress.activeIndex << " active_block="
                  << ((transferProgress.activeIndex < transferPlan.size())
                          ? transferPlan[transferProgress.activeIndex].name
                          : std::string("-"))
