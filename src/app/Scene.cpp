...
namespace scrap {
    ...
    void Scene::addEntity(scrap::Entity&& entity) {
        entities.push_back(std::move(entity));
    }

    scrap::Entity* Scene::getEntity(const std::string& name) {
    ...
    bool Scene::saveToFile(const std::string& filePath) const {
        using namespace scrap::modeling;
    ...
    void Scene::loadGLTFNodeHierarchy(
        ...
        entity.addComponent<TransformComponent>(
            modeling::Transform::fromMatrix(node->getWorldMatrix()));
        ...
        if (node->hasCloth()) {
            ...
            if (meshMaterialPairs.size() == 1 && meshMaterialPairs[0].mesh) {
                entity.addComponent<ClothComponent>(meshMaterialPairs[0].mesh,
        ...
        if (node->hasLight()) {
            ...
            if (info.type == modeling::LightInfo::Type::Point) {
                ...
                glm::vec3 direction = glm::normalize(node->getTransform().getRotation() *
                                                     glm::vec3(0.0f, 0.0f, -1.0f));
                entity.addComponent<SpotLightComponent>(info.color, info.intensity, info.range,
                                                        direction, info.innerConeAngle,
                                                        info.outerConeAngle);
    ...
    const std::vector<GPULight>& Scene::collectGPULights() {
        ...
        for (auto& entity : entities) {
            ...
            if (lc->getLightType() == LightType::Directional ||
                lc->getLightType() == LightType::Spot) {
