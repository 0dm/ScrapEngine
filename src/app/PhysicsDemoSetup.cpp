...
namespace scrap::physics_demo {

    namespace {
        bool isStaticTableMesh(const MeshRendererComponent* meshRenderer) {
            return meshRenderer && meshRenderer->getMesh() &&
                   meshRenderer->getMesh()->getVertexCount() == 4;
        }
    } // namespace
    ...
    void configureRigidBodyFromEntity(Entity& entity, RigidBodyComponent& rigidBody) {
        ...
        if (!isStaticTableMesh(meshRenderer)) {
            invMass = RigidBodyComponent::scaledMeshInvMass(meshRenderer->getMesh(),
                                                            transform->getScale());
        ...
        rigidBody.setMassProperties(
            invMass, RigidBodyComponent::meshInvInertiaTensor(meshRenderer->getMesh(), centerOfMass,
                                                              invMass, transform->getScale()));
    }
    ...
    RigidBodyComponent* ensureEntityRigidBody(Entity& entity) {
        ...
        if (!rigidBody) {
            entity.addComponent<RigidBodyComponent>(transform->getTranslation(), glm::vec3(0.0f),
                                                    transform->getRotation(), glm::vec3(0.0f));
