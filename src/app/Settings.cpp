...
namespace scrap {
    ...
    void SettingsManager::load(const std::string& path) {
        ...
        if (!file.is_open()) {
            SCRAP_LOG("Settings", "No config file found at {}, using defaults", filePath);
            save();
            return;
        }
        ...
        try {
            ...
            SCRAP_LOG("Settings", "Loaded settings from {}", filePath);
        } catch (const nlohmann::json::exception& e) {
            SCRAP_LOG("Settings", "Failed to parse {}: {}, using defaults", filePath, e.what());
    ...
    void SettingsManager::save() const {
        ...
        if (file.is_open()) {
            ...
            SCRAP_LOG_VERBOSE("Settings", "Settings saved to {}", filePath);
        } else {
            SCRAP_LOG("Settings", "Failed to save settings to {}", filePath);
        }
    }
