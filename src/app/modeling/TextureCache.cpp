...
namespace scrap {
    namespace modeling {
        ...
        std::shared_ptr<Texture> TextureCache::getTexture(const std::string& path, TextureType type,
                                                          bool sRGB) {
            const std::string key = makeCacheKey(path, type, sRGB);
        ...
        std::shared_ptr<Texture> TextureCache::getEmbeddedTexture(
            const std::vector<unsigned char>& data, int width, int height, int channels,
            TextureType type, bool sRGB) {
            // Generate hash-based key for embedded texture
            std::string key = makeCacheKey(generateEmbeddedKey(data), type, sRGB);
            ...
            auto texture =
                std::make_shared<Texture>(data, width, height, channels, type, sRGB, key);
            cache[key] = texture;
            return texture;
        }
        ...
        std::string TextureCache::makeCacheKey(const std::string& baseKey, TextureType type,
                                               bool sRGB) const {
            return baseKey + "#" + std::to_string(static_cast<int>(type)) +
                   (sRGB ? "#srgb" : "#linear");
        }
        ...
        void TextureCache::initializeDefaultTextures() {
            ...
            // R=occlusion(1.0), G=roughness(0.5), B=metallic(0.0)
            {
                ...
                defaultTextures[TextureType::MetallicRoughness] = std::make_shared<Texture>(
                    metallicRoughnessPixel, 1, 1, 4, TextureType::MetallicRoughness, false,
