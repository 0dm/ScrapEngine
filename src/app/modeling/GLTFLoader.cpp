...
namespace scrap {
    namespace modeling {
        ...
        GLTFLoader::GLTFLoader() : currentGltfModel(nullptr) {
        }

        GLTFLoader::GLTFLoader(const LoadOptions& options)
            : options(options), currentGltfModel(nullptr) {
        }
        ...
        std::shared_ptr<Model> GLTFLoader::loadModel(const std::string& filePath,
                                                     size_t sceneIndex) {
            primitiveCache.clear();
            materialCache.clear();
            tinygltf::Model gltfModel;
        ...
        std::shared_ptr<Model> GLTFLoader::processScene(const tinygltf::Model& gltfModel,
                                                        int sceneIndex) {
        ...
        std::shared_ptr<ModelNode> GLTFLoader::processNode(const tinygltf::Model& gltfModel,
                                                           int nodeIndex) {
            ...
            if (gltfNode.mesh >= 0 && gltfNode.mesh < static_cast<int>(gltfModel.meshes.size())) {
                ...
                for (size_t primitiveIndex = 0; primitiveIndex < gltfMesh.primitives.size();
                    ...
                    auto mesh = processPrimitive(gltfModel, gltfNode.mesh,
                                                 static_cast<int>(primitiveIndex), primitive);
                    if (mesh) {
                        // Get material
        ...
        Transform GLTFLoader::extractTransform(const tinygltf::Node& gltfNode) {
            ...
            if (!gltfNode.matrix.empty() && gltfNode.matrix.size() == 16) {
                ...
                if (!gltfNode.rotation.empty() && gltfNode.rotation.size() == 4) {
                    ...
                    transform.setRotation(glm::quat(static_cast<float>(gltfNode.rotation[3]), // w
                ...
                if (!gltfNode.scale.empty() && gltfNode.scale.size() == 3) {
                    transform.setScale(glm::vec3(static_cast<float>(gltfNode.scale[0]),
        ...
        std::shared_ptr<Material> GLTFLoader::processMaterial(const tinygltf::Model& gltfModel,
                                                              int materialIndex) {
            if (materialIndex < 0 ||
                materialIndex >= static_cast<int>(gltfModel.materials.size())) {
                return nullptr;
            }
            ...
            if (gltfMaterial.pbrMetallicRoughness.baseColorTexture.index >= 0) {
                auto texture = processTexture(
                    gltfModel, gltfMaterial.pbrMetallicRoughness.baseColorTexture.index,
            ...
            if (gltfMaterial.pbrMetallicRoughness.metallicRoughnessTexture.index >= 0) {
                auto texture = processTexture(
                    gltfModel, gltfMaterial.pbrMetallicRoughness.metallicRoughnessTexture.index,
            ...
            props.baseColorFactor = glm::vec4(static_cast<float>(bc[0]), static_cast<float>(bc[1]),
            ...
            props.emissiveFactor = glm::vec3(static_cast<float>(ef[0]), static_cast<float>(ef[1]),
        ...
        std::shared_ptr<Texture> GLTFLoader::processTexture(const tinygltf::Model& gltfModel,
                                                            int textureIndex, TextureType type,
                                                            bool sRGB) {
        ...
        std::shared_ptr<Texture> GLTFLoader::processImage(const tinygltf::Model& gltfModel,
                                                          int imageIndex, TextureType type,
                                                          bool sRGB) {
            ...
            if (!gltfImage.uri.empty()) {
                ...
                std::filesystem::path imagePath =
                    std::filesystem::path(baseDirectory) / gltfImage.uri;
                return textureCache.getTexture(imagePath.string(), type, sRGB);
            } else if (!gltfImage.image.empty()) {
                // Embedded image
                return textureCache.getEmbeddedTexture(gltfImage.image, gltfImage.width,
                                                       gltfImage.height, gltfImage.component, type,
                                                       sRGB);
            }
        ...
        void GLTFLoader::parseLightsExtension(const tinygltf::Model& gltfModel) {
            ...
            if (extIt == gltfModel.extensions.end())
                return;

            const auto& extValue = extIt->second;
            if (!extValue.Has("lights") || !extValue.Get("lights").IsArray())
                return;
            ...
            for (size_t i = 0; i < lightsArray.ArrayLen(); ++i) {
                ...
                if (lightVal.Has("color") && lightVal.Get("color").IsArray()) {
                    ...
                    if (c.ArrayLen() >= 3) {
                        info.color = glm::vec3(
                            static_cast<float>(c.Get(0).IsNumber() ? c.Get(0).GetNumberAsDouble()
                                                                   : 1.0),
                            static_cast<float>(c.Get(1).IsNumber() ? c.Get(1).GetNumberAsDouble()
                                                                   : 1.0),
                            static_cast<float>(c.Get(2).IsNumber() ? c.Get(2).GetNumberAsDouble()
                                                                   : 1.0));
                    }
                }
                ...
                if (lightVal.Has("intensity") && lightVal.Get("intensity").IsNumber()) {
                    info.intensity =
                        static_cast<float>(lightVal.Get("intensity").GetNumberAsDouble());
                }
                ...
                if (lightVal.Has("spot") && lightVal.Get("spot").IsObject()) {
                    ...
                    if (spot.Has("innerConeAngle") && spot.Get("innerConeAngle").IsNumber()) {
                        info.innerConeAngle =
                            static_cast<float>(spot.Get("innerConeAngle").GetNumberAsDouble());
                    }
                    if (spot.Has("outerConeAngle") && spot.Get("outerConeAngle").IsNumber()) {
                        info.outerConeAngle =
                            static_cast<float>(spot.Get("outerConeAngle").GetNumberAsDouble());
                    }
                }
        ...
        void GLTFLoader::applyNodeLight(const tinygltf::Node& gltfNode,
                                        std::shared_ptr<ModelNode> node) {
            ...
            const auto& extValue = extIt->second;
            if (!extValue.Has("light") || !extValue.Get("light").IsInt())
                return;

            int lightIndex = extValue.Get("light").GetNumberAsInt();
            if (lightIndex < 0 || lightIndex >= static_cast<int>(parsedLights.size()))
                return;

            node->setLightInfo(parsedLights[lightIndex]);
        }

        void GLTFLoader::applyNodeCloth(const tinygltf::Node& gltfNode,
                                        std::shared_ptr<ModelNode> node) {
            const tinygltf::Value* extPtr = nullptr;
            if (auto extIt = gltfNode.extensions.find("SCRAP_cloth");
                extIt != gltfNode.extensions.end()) {
                extPtr = &extIt->second;
            } else if (auto legacyIt = gltfNode.extensions.find("SAUCE_cloth");
                       legacyIt != gltfNode.extensions.end()) {
                extPtr = &legacyIt->second;
            }
            if (!extPtr) {
                return;
            }

            const auto& extValue = *extPtr;
            ClothInfo clothInfo;
            ...
            } else if (extValue.Has("solverSubsteps") &&
                       extValue.Get("solverSubsteps").IsNumber()) {
                clothInfo.settings.solverSubsteps =
                    static_cast<int>(extValue.Get("solverSubsteps").GetNumberAsDouble());
            }

            if (extValue.Has("pinnedParticleIndices") &&
                extValue.Get("pinnedParticleIndices").IsArray()) {
