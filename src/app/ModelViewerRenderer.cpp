...
void ModelViewerRenderer::loadModel(
...
void ModelViewerRenderer::uploadMeshToGPU(
  ...
  sauce::BufferUtils::createBuffer(
      ...
      gpuData.vertexBufferMemory
  );

  sauce::BufferUtils::copyBuffer(logicalDevice, commandPool, *pQueue, stagingVertexBuffer, gpuData.vertexBuffer, vertexBufferSize);
  ...
  sauce::BufferUtils::createBuffer(
      ...
      vk::MemoryPropertyFlagBits::eDeviceLocal,
      gpuData.indexBuffer,
      gpuData.indexBufferMemory
  );

  sauce::BufferUtils::copyBuffer(logicalDevice, commandPool, *pQueue, stagingIndexBuffer, gpuData.indexBuffer, indexBufferSize);
...
void ModelViewerRenderer::createUniformBuffers(
    const sauce::PhysicalDevice& physicalDevice,
    const sauce::LogicalDevice& logicalDevice) {
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
...
void ModelViewerRenderer::transitionImageLayout(
    uint32_t imageIndex,
...
void ModelViewerRenderer::recordCommandBuffer(uint32_t imageIndex, sauce::ImGuiRenderer* imguiRenderer) {
  ...
  for (const auto& meshData : meshBuffers) {
    ...
    commandBuffers[frameIndex].bindIndexBuffer(*meshData.indexBuffer, 0, vk::IndexType::eUint32);
...
void ModelViewerRenderer::updateUniformBuffer(uint32_t curImage, const sauce::Camera& camera, float modelRotationAngle) {
...
void ModelViewerRenderer::drawFrame(
    const sauce::LogicalDevice& logicalDevice,
    const sauce::Camera& camera,
