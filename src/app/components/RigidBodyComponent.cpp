...
namespace scrap {
    ...
    float RigidBodyComponent::scaledMeshInvMass(std::shared_ptr<modeling::Mesh> m,
    ...
    glm::mat3 RigidBodyComponent::meshInvInertiaTensor(std::shared_ptr<modeling::Mesh> m,
    ...
    glm::mat3 RigidBodyComponent::meshInvInertiaTensor(std::shared_ptr<modeling::Mesh> m,
        ...
        if (ixx > 1e-8f)
            invInertia[0][0] = 1.0f / ixx;
        if (iyy > 1e-8f)
            invInertia[1][1] = 1.0f / iyy;
        if (izz > 1e-8f)
            invInertia[2][2] = 1.0f / izz;
        return invInertia;
    }
