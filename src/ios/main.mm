...
class IOSPlatformView final : public sauce::platform::PlatformView {
public:
  ...
  vk::Extent2D getFramebufferExtent() const override {
  ...
  void prepareImGuiFrame(ImGuiIO& io, float deltaTime) override {
    io.DisplaySize = ImVec2(
        static_cast<float>(nativeView.bounds.size.width),
    ...
    addKeyEvent(io, ImGuiKey_Escape, sauce::platform::Key::Escape);
    addKeyEvent(io, ImGuiKey_GraveAccent, sauce::platform::Key::GraveAccent);
    addKeyEvent(io, ImGuiKey_Delete, sauce::platform::Key::DeleteKey);
    addKeyEvent(io, ImGuiKey_F, sauce::platform::Key::F);
    addKeyEvent(io, ImGuiKey_E, sauce::platform::Key::E);
    addKeyEvent(io, ImGuiKey_R, sauce::platform::Key::R);
    addKeyEvent(io, ImGuiKey_O, sauce::platform::Key::O);
    addKeyEvent(io, ImGuiKey_N, sauce::platform::Key::N);
    addKeyEvent(io, ImGuiKey_P, sauce::platform::Key::P);
    addKeyEvent(io, ImGuiKey_Space, sauce::platform::Key::Space);
    addKeyEvent(io, ImGuiKey_LeftShift, sauce::platform::Key::LeftShift);
    addKeyEvent(io, ImGuiKey_RightShift, sauce::platform::Key::RightShift);
    addKeyEvent(io, ImGuiKey_LeftCtrl, sauce::platform::Key::LeftControl);
    addKeyEvent(io, ImGuiKey_RightCtrl, sauce::platform::Key::RightControl);
    addKeyEvent(io, ImGuiKey_W, sauce::platform::Key::W);
    addKeyEvent(io, ImGuiKey_A, sauce::platform::Key::A);
    addKeyEvent(io, ImGuiKey_S, sauce::platform::Key::S);
    addKeyEvent(io, ImGuiKey_D, sauce::platform::Key::D);
    ...
    if (cursorCaptured) {
      ...
      io.AddMouseButtonEvent(
          0,
          inputState.pointer.buttons[static_cast<std::size_t>(sauce::platform::MouseButton::Left)].down);
    }
  ...
  void setCursorCaptured(bool captured) override {
    ...
    if (captured) {
      auto& leftButton =
          inputState.pointer.buttons[static_cast<std::size_t>(sauce::platform::MouseButton::Left)];
  ...
  void updateDrawableSize(CGSize size, CGFloat scale) {
    ...
    metalLayer.drawableSize = CGSizeMake(size.width * contentScaleFactor, size.height * contentScaleFactor);
    framebufferExtent = {
  ...
  void handleTouches(NSSet<UITouch*>* touches, UIView* view, UITouchPhase phase) {
    ...
    switch (phase) {
      case UITouchPhaseBegan:
        inputState.pointer.buttons[static_cast<std::size_t>(sauce::platform::MouseButton::Left)].down = true;
        inputState.pointer.buttons[static_cast<std::size_t>(sauce::platform::MouseButton::Left)].pressed = true;
        break;
      ...
      case UITouchPhaseCancelled:
        inputState.pointer.buttons[static_cast<std::size_t>(sauce::platform::MouseButton::Left)].released =
            inputState.pointer.buttons[static_cast<std::size_t>(sauce::platform::MouseButton::Left)].down;
        inputState.pointer.buttons[static_cast<std::size_t>(sauce::platform::MouseButton::Left)].down = false;
  ...
  void setKey(UIKeyboardHIDUsage usage, bool pressed) {
    ...
    const std::size_t index = sauce::platform::keyIndex(*key);
    ...
    inputState.shiftDown =
        inputState.keys[sauce::platform::keyIndex(sauce::platform::Key::LeftShift)] ||
        inputState.keys[sauce::platform::keyIndex(sauce::platform::Key::RightShift)];
    inputState.controlDown =
        inputState.keys[sauce::platform::keyIndex(sauce::platform::Key::LeftControl)] ||
        inputState.keys[sauce::platform::keyIndex(sauce::platform::Key::RightControl)];
  }

private:
  static std::optional<sauce::platform::Key> mapKey(UIKeyboardHIDUsage usage) {
    switch (usage) {
  ...
  void addKeyEvent(ImGuiIO& io, ImGuiKey imguiKey, sauce::platform::Key platformKey) {
    io.AddKeyEvent(
        imguiKey,
        inputState.keys[sauce::platform::keyIndex(platformKey)]);
  }

  UIView* nativeView = nil;
  CAMetalLayer* metalLayer = nil;
  sauce::platform::InputState inputState{};
...
@interface SauceMetalView : UIView <UIKeyInput> {
@public
  sauce_ios::IOSPlatformView* platformBridge;
}
@end

@implementation SauceMetalView
...
- (instancetype)initWithFrame:(CGRect)frame {
  ...
  UITapGestureRecognizer* toggleCaptureGesture =
      [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleCursorCaptureToggle:)];
...
@interface SauceViewController : UIViewController
@end

@implementation SauceViewController {
  std::shared_ptr<sauce_ios::IOSPlatformView> _platformView;
  std::unique_ptr<sauce::SauceEngineApp> _app;
...
- (void)loadView {
  self.view = [[SauceMetalView alloc] initWithFrame:UIScreen.mainScreen.bounds];
}
...
- (void)viewDidLoad {
  ...
  auto* metalView = (SauceMetalView*)self.view;
  _platformView = std::make_shared<sauce_ios::IOSPlatformView>(metalView);
  metalView->platformBridge = _platformView.get();
  [self.view setNeedsLayout];
  [self.view layoutIfNeeded];

  _app = std::make_unique<sauce::SauceEngineApp>();

  try {
    const vk::Extent2D extent = _platformView->getFramebufferExtent();
    _app->initialize(
        *_platformView,
        extent.width > 0 ? extent.width : 1280u,
        extent.height > 0 ? extent.height : 720u);
  } catch (const std::exception& error) {
    NSLog(@"SauceEngine initialization failed: %s", error.what());
    _failed = YES;
  }
}
...
- (void)step:(CADisplayLink*)displayLink {
  ...
  const float deltaTime =
      _lastTimestamp > 0.0 ? static_cast<float>(displayLink.timestamp - _lastTimestamp) : (1.0f / 60.0f);
  _lastTimestamp = displayLink.timestamp;

  try {
    _app->tick(deltaTime);
  } catch (const std::exception& error) {
    NSLog(@"SauceEngine frame failed: %s", error.what());
    _failed = YES;
    [_displayLink invalidate];
    _displayLink = nil;
  }
...
@interface SauceAppDelegate : UIResponder <UIApplicationDelegate>
@property(nonatomic, strong) UIWindow* window;
@end

@implementation SauceAppDelegate
...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions {
  ...
  self.window.rootViewController = [SauceViewController new];
...
int main(int argc, char* argv[]) {
  @autoreleasepool {
    ...
    return UIApplicationMain(argc, argv, nil, NSStringFromClass([SauceAppDelegate class]));
  }
}
