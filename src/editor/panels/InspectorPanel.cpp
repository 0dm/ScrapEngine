...
namespace scrap::editor {
    ...
    namespace {
        ...
        ClothComponent* findClothComponentForMesh(
            scrap::Entity& entity, const std::shared_ptr<scrap::modeling::Mesh>& mesh) {
            if (!mesh) {
                return nullptr;
            }
    ...
    InspectorPanel::InspectorPanel(EditorApp& app) : EditorPanel("Inspector", app) {
    }
    ...
    static bool drawVec3Control(const std::string& label, glm::vec3& values, float resetValue,
                                float speed) {
        bool modified = false;
        ...
        if (ImGui::InputFloat("##X", &values.x, 0.0f, 0.0f, "%.2f", inputFlags))
            modified = true;
        ...
        ImGui::SetNextItemWidth(fieldWidth);
        if (ImGui::InputFloat("##Y", &values.y, 0.0f, 0.0f, "%.2f", inputFlags))
            modified = true;
        ImGui::PopStyleColor(3);
        ...
        ImGui::SetNextItemWidth(fieldWidth);
        if (ImGui::InputFloat("##Z", &values.z, 0.0f, 0.0f, "%.2f", inputFlags))
            modified = true;
        ImGui::PopStyleColor(3);
    ...
    void InspectorPanel::drawTransformSection(scrap::Entity& entity) {
    ...
    void InspectorPanel::drawMeshRendererSection(scrap::Entity& entity) {
        ...
        for (size_t i = 0; i < mrcs.size(); ++i) {
            ...
            std::string headerLabel =
                (mrcs.size() == 1) ? "Mesh Renderer" : "Mesh " + std::to_string(i);
            ...
            if (ImGui::CollapsingHeader(headerLabel.c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip(
                        "Drag a .gltf/.glb from the Asset Browser to assign or replace");
                ...
                if (material) {
                    ...
                    if (ImGui::SliderFloat("Roughness", &props.roughnessFactor, 0.0f, 1.0f,
                                           "%.2f")) {
                    }
    ...
    void InspectorPanel::drawClothSection(scrap::Entity& entity) {
        auto clothComponents = entity.getComponents<ClothComponent>();
        if (clothComponents.empty())
            return;
        ...
        for (size_t i = 0; i < clothComponents.size(); ++i) {
            ...
            const std::string headerLabel =
                (clothComponents.size() == 1) ? "Cloth" : "Cloth " + std::to_string(i);
            ...
            if (ImGui::CollapsingHeader(headerLabel.c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (sourceMesh) {
                    ImGui::Text("Source Mesh: %zu vertices / %zu indices",
                                sourceMesh->getVertexCount(), sourceMesh->getIndexCount());
                } else {
                ...
                if (cloth->hasClothData()) {
                    ...
                    if (ImGui::TreeNode("Particle Preview")) {
                        const size_t previewCount =
                            std::min<size_t>(clothData->particles.size(), 4);
                        for (size_t particleIndex = 0; particleIndex < previewCount;
                             ++particleIndex) {
                            const auto& particle = clothData->particles[particleIndex];
                            ImGui::BulletText(
                                "#%zu pos=(%.2f, %.2f, %.2f) pred=(%.2f, %.2f, %.2f) invMass=%.2f "
                                "pinned=%s",
                                particleIndex, particle.position.x, particle.position.y,
                                particle.position.z, particle.predictedPosition.x,
                                particle.predictedPosition.y, particle.predictedPosition.z,
                                particle.invMass, particle.pinned ? "true" : "false");
                        }
                        ImGui::TreePop();
                    }
                    ...
                    if (ImGui::TreeNode("Edge Preview")) {
                        const size_t previewCount =
                            std::min<size_t>(clothData->topology.edges.size(), 4);
                        for (size_t edgeIndex = 0; edgeIndex < previewCount; ++edgeIndex) {
                            const auto& edge = clothData->topology.edges[edgeIndex];
                            ImGui::BulletText("#%zu [%u, %u] triangles=(%u, %u)", edgeIndex,
                                              edge.particleIndices[0], edge.particleIndices[1],
                    ...
                    if (ImGui::TreeNode("Stretch Preview")) {
                        const size_t previewCount =
                            std::min<size_t>(clothData->stretchConstraints.size(), 4);
                        for (size_t stretchIndex = 0; stretchIndex < previewCount; ++stretchIndex) {
                            ...
                            ImGui::BulletText("#%zu [%u, %u] restLength=%.4f", stretchIndex,
                                              stretch.particleIndices[0],
                                              stretch.particleIndices[1], stretch.restLength);
                        }
                        ImGui::TreePop();
                    }
                    ...
                    if (ImGui::TreeNode("Bend Preview")) {
                        const size_t previewCount =
                            std::min<size_t>(clothData->bendConstraints.size(), 4);
                        for (size_t bendIndex = 0; bendIndex < previewCount; ++bendIndex) {
                            const auto& bend = clothData->bendConstraints[bendIndex];
                            ImGui::BulletText("#%zu edge=[%u, %u] opp=[%u, %u] restAngle=%.4f",
                                              bendIndex, bend.sharedEdgeParticleIndices[0],
                                              bend.sharedEdgeParticleIndices[1],
                                              bend.oppositeParticleIndices[0],
                                              bend.oppositeParticleIndices[1], bend.restAngle);
                        }
                        ImGui::TreePop();
                    }
