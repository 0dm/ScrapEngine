...
namespace physics {
    namespace {
        ...
        std::array<uint32_t, 3> getTriangle(const std::vector<uint32_t>& triangleIndices,
        ...
        uint32_t findOppositeParticle(const std::array<uint32_t, 3>& triangle, uint32_t edgeA,
                                      uint32_t edgeB) {
        ...
        float computeRestBendAngle(const ClothData& clothData, uint32_t edgeA, uint32_t edgeB,
                                   uint32_t oppositeA, uint32_t oppositeB) {
    ...
    std::optional<ClothData> buildClothDataFromMesh(const scrap::modeling::Mesh& mesh,
            ...
            for (const auto& triangleEdge : triangleEdges) {
                ...
                if (edgeIt == edgeMap.end()) {
                    const uint32_t edgeIndex =
                        static_cast<uint32_t>(clothData.topology.edges.size());
        ...
        for (const ClothEdge& edge : clothData.topology.edges) {
            ...
            clothData.stretchConstraints.emplace_back(
                edge.particleIndices[0], edge.particleIndices[1], glm::length(p1 - p0));
        }

        clothData.bendConstraints.reserve(clothData.topology.edges.size());
        for (uint32_t edgeIndex = 0;
             edgeIndex < static_cast<uint32_t>(clothData.topology.edges.size()); ++edgeIndex) {
            const ClothEdge& edge = clothData.topology.edges[edgeIndex];
            if (edge.isBoundary()) {
                continue;
            }

            const auto triangleA =
                getTriangle(clothData.topology.triangleIndices, edge.adjacentTriangleIndices[0]);
            const auto triangleB =
                getTriangle(clothData.topology.triangleIndices, edge.adjacentTriangleIndices[1]);

            const uint32_t oppositeA =
                findOppositeParticle(triangleA, edge.particleIndices[0], edge.particleIndices[1]);
            const uint32_t oppositeB =
                findOppositeParticle(triangleB, edge.particleIndices[0], edge.particleIndices[1]);
            ...
            clothData.bendConstraints.emplace_back(
                edgeIndex, edge.particleIndices[0], edge.particleIndices[1], oppositeA, oppositeB,
                computeRestBendAngle(clothData, edge.particleIndices[0], edge.particleIndices[1],
                                     oppositeA, oppositeB),
                edge.adjacentTriangleIndices[0], edge.adjacentTriangleIndices[1]);
        }
