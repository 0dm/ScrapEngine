...
namespace physics {
    ...
    namespace {
        ...
        std::unique_ptr<SphereBVHNode> buildNode(std::vector<TriangleInfo>& triangles, size_t start,
                                                 size_t end) {
            ...
            for (size_t i = start; i < end; ++i) {
                maxRadiusSq =
                    std::max(maxRadiusSq, glm::length2(node->sphere.center - triangles[i].v0));
                maxRadiusSq =
                    std::max(maxRadiusSq, glm::length2(node->sphere.center - triangles[i].v1));
                maxRadiusSq =
                    std::max(maxRadiusSq, glm::length2(node->sphere.center - triangles[i].v2));
            }
        ...
        std::unique_ptr<SphereBVHNode> buildBVH(
            std::vector<std::unique_ptr<SphereBVHNode>>& spheres, int start, int end) {
            int count = end - start;
            if (count <= 0) {
                return nullptr;
            }
            ...
            std::nth_element(spheres.begin() + start, spheres.begin() + mid, spheres.begin() + end,
                             [axis](const std::unique_ptr<SphereBVHNode>& a,
                                    const std::unique_ptr<SphereBVHNode>& b) {
    ...
    static void reduceManifold(std::vector<ContactInfo>& contacts) {
        if (contacts.size() <= MAX_MANIFOLD_CONTACTS)
            return;
        ...
        if (MAX_MANIFOLD_CONTACTS >= 4) {
            glm::vec3 triNormal = glm::cross(reduced[1].contactPoint - reduced[0].contactPoint,
            ...
            for (size_t i = 0; i < contacts.size(); ++i) {
                float d = std::abs(
                    glm::dot(contacts[i].contactPoint - reduced[0].contactPoint, triNormal));
                if (d > maxDist) {
                    maxDist = d;
                    fourthIdx = i;
                }
            ...
            if (maxDist < 1e-6f) {
                glm::vec3 centroid =
                    (reduced[0].contactPoint + reduced[1].contactPoint + reduced[2].contactPoint) /
                    3.0f;
    ...
    bool SphereBVHNode::checkCollision(const Collider& collider,
    ...
    std::unique_ptr<SphereBVHNode> SphereBVHNode::fromMesh(scrap::modeling::Mesh& mesh) {
        ...
        for (size_t i = 0; i < numTriangles; ++i) {
            ...
            triangles.push_back({.idx = static_cast<uint32_t>(i),
                                 .v0 = p0,
                                 .v1 = p1,
                                 .v2 = p2,
    ...
    // SphereBVH
    SphereBVH SphereBVH::fromScene(const scrap::Scene& scene) {

        // make BVH for every mesh
        std::vector<std::unique_ptr<SphereBVHNode>> spheres;
        auto& entities = scene.getEntities();
        for (auto& entity : entities) {
            auto mesh_renderer = entity.getComponent<scrap::MeshRendererComponent>();
