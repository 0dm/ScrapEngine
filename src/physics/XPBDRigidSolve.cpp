...
namespace physics {
    ...
    namespace {
        ...
        class RigidSubstep {
          public:
            explicit RigidSubstep(const std::vector<scrap::RigidBodyComponent*>& rigidBodies) {
            ...
            void begin(const std::vector<scrap::RigidBodyComponent*>& rigidBodies,
                       float deltaTime) {
                vertices.clear();
                ...
                for (size_t i = 0; i < bodyCount; ++i) {
                    ...
                    if (i < previousOrientations.size()) {
                        rigidBody->setAngularVelocity(clampMagnitude(
                            angularVelocityFromSolvedOrientation(previousOrientations[i],
                                                                 solvedOrientation, deltaTime),
            ...
            std::vector<scrap::RigidBodyComponent*> rigidBodiesState;
        ...
        glm::mat3 worldInvInertiaTensor(const scrap::RigidBodyComponent& rigidBody) {
        ...
        glm::vec3 velocityAtPoint(const scrap::RigidBodyComponent& rigidBody,
                                  const glm::vec3& offset) {
            return rigidBody.getVelocity() + glm::cross(rigidBody.getAngularVelocity(), offset);
        }

        float angularEffectiveMass(const scrap::RigidBodyComponent& rigidBody,
                                   const glm::vec3& offset, const glm::vec3& direction) {
        ...
        void applyImpulse(scrap::RigidBodyComponent& rigidBody, const glm::vec3& impulse,
                          const glm::vec3& offset) {
            if (rigidBody.getInvMass() <= 1e-8f) {
                return;
            }
            ...
            rigidBody.setAngularVelocity(rigidBody.getAngularVelocity() +
                                         worldInvInertia * glm::cross(offset, impulse));
        }
    ...
    void XPBDSolver::captureCollisionLambdaWarmStart(
        ...
        for (const auto& constraint : constraints) {
            if (constraint.indexA >= rigidBodies.size() ||
                constraint.indexB >= rigidBodies.size()) {
                continue;
            }
    ...
    void XPBDSolver::solvePositions(std::vector<scrap::RigidBodyComponent*>& rigidBodies,
                                    float deltatime) {
        if (deltatime <= 0.0f) {
            return;
        }

        std::vector<scrap::RigidBodyComponent*> simulatedBodies;
        ...
        for (auto* rigidBody : rigidBodies) {
            ...
            auto* meshRenderer =
                owner ? owner->getComponent<scrap::MeshRendererComponent>() : nullptr;
        ...
        for (auto* rigidBody : simulatedBodies) {
            ...
            if (linSpeedSq < kAutoSleepLinear * kAutoSleepLinear &&
                angSpeedSq < kAutoSleepAngular * kAutoSleepAngular && rigidBody->canBeDynamic()) {
                rigidBody->sleep();
            }
        }
