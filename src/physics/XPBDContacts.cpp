...
namespace physics {
    ...
    namespace {
        ...
        struct CollisionBody {
            uint32_t index = 0;
            scrap::RigidBodyComponent* rigidBody = nullptr;
            const scrap::modeling::Mesh* mesh = nullptr;
            ...
            float halfExtent(int axisIndex) const {
                return boxHalfExtents[axisIndex];
            }
            std::array<glm::vec3, 8> corners() const;
            std::array<glm::vec3, 4> faceCorners(int normalAxisIndex,
        ...
        class MeshCollisionCache {
          public:
            ...
            const SphereBVHNode* getBVHRoot(const scrap::modeling::Mesh* mesh) {
                if (!mesh) {
                    return nullptr;
                }
                ...
                if (it == bvhRoots.end()) {
                    auto built = SphereBVHNode::fromMesh(*const_cast<scrap::modeling::Mesh*>(mesh));
            ...
            std::optional<BoxShape> getBoxShape(const scrap::modeling::Mesh* mesh) {
                if (!mesh) {
                    return std::nullopt;
                }
                ...
                std::array<bool, 8> cornerSeen = {false, false, false, false,
                ...
                const bool allCornersPresent = std::all_of(cornerSeen.begin(), cornerSeen.end(),
            ...
            std::optional<PlaneShape> getPlaneShape(const scrap::modeling::Mesh* mesh) {
                if (!mesh) {
                    return std::nullopt;
                }
            ...
            float getBroadPhasePadding(const scrap::modeling::Mesh* mesh) {
                if (!mesh) {
                    return 0.02f;
                }
            ...
            float getContactMargin(const scrap::modeling::Mesh* mesh) const {
                if (!mesh) {
                    return 0.008f;
                }
            ...
            const std::vector<uint32_t>& getSampleVertexIndices(const scrap::modeling::Mesh* mesh) {
                ...
                for (uint32_t i = 1; i < static_cast<uint32_t>(vertices.size()); ++i) {
                    ...
                    if (position.x < vertices[extrema[0]].position.x)
                        extrema[0] = i;
                    if (position.x > vertices[extrema[1]].position.x)
                        extrema[1] = i;
                    if (position.y < vertices[extrema[2]].position.y)
                        extrema[2] = i;
                    if (position.y > vertices[extrema[3]].position.y)
                        extrema[3] = i;
                    if (position.z < vertices[extrema[4]].position.z)
                        extrema[4] = i;
                    if (position.z > vertices[extrema[5]].position.z)
                        extrema[5] = i;
                }
                ...
                const size_t remainingSlots =
                    kMaxSampleVertices > indices.size() ? (kMaxSampleVertices - indices.size()) : 0;
                const size_t stride = remainingSlots > 0
                                          ? std::max<size_t>(1, vertices.size() / remainingSlots)
                                          : vertices.size();

                for (size_t i = 0; i < vertices.size() && indices.size() < kMaxSampleVertices;
                     i += stride) {
                    addSampleIndex(static_cast<uint32_t>(i));
                }

                for (uint32_t i = 0; i < static_cast<uint32_t>(vertices.size()) &&
                                     indices.size() < kMaxSampleVertices;
                     ++i) {
                    addSampleIndex(i);
                }
          ...
          private:
            float averageEdgeLengthFor(const scrap::modeling::Mesh* mesh, float fallback) const {
                if (!mesh) {
                    return fallback;
                }
                ...
                return edgeCount > 0 ? static_cast<float>(edgeSum / static_cast<double>(edgeCount))
                                     : fallback;
            }

            std::unordered_map<const scrap::modeling::Mesh*, std::unique_ptr<SphereBVHNode>>
                bvhRoots;
            std::unordered_map<const scrap::modeling::Mesh*, float> contactRadii;
            std::unordered_map<const scrap::modeling::Mesh*, std::optional<BoxShape>> boxShapes;
            std::unordered_map<const scrap::modeling::Mesh*, std::optional<PlaneShape>> planeShapes;
            std::unordered_map<const scrap::modeling::Mesh*, std::vector<uint32_t>>
                sampleVertexIndices;
        };
        ...
        SphereCollider transformSphere(const SphereCollider& localSphere,
                                       const scrap::RigidBodyComponent& body) {
            SphereCollider worldSphere;
            worldSphere.center =
                body.getPosition() +
                body.getOrientation() * scalePoint(localSphere.center, body.getScale());
        ...
        glm::vec3 CollisionBody::axis(int axisIndex) const {
            switch (axisIndex) {
            case 0:
                return glm::normalize(rigidBody->getOrientation() * glm::vec3(1.0f, 0.0f, 0.0f));
            case 1:
                return glm::normalize(rigidBody->getOrientation() * glm::vec3(0.0f, 1.0f, 0.0f));
            default:
                return glm::normalize(rigidBody->getOrientation() * glm::vec3(0.0f, 0.0f, 1.0f));
            }
        }
        ...
        std::array<glm::vec3, 8> CollisionBody::corners() const {
            ...
            const glm::vec3 ex =
                rigidBody->getOrientation() * glm::vec3(boxHalfExtents.x, 0.0f, 0.0f);
            const glm::vec3 ey =
                rigidBody->getOrientation() * glm::vec3(0.0f, boxHalfExtents.y, 0.0f);
            const glm::vec3 ez =
                rigidBody->getOrientation() * glm::vec3(0.0f, 0.0f, boxHalfExtents.z);

            size_t cornerIndex = 0;
            for (int sx : {-1, 1}) {
                for (int sy : {-1, 1}) {
                    for (int sz : {-1, 1}) {
                        result[cornerIndex++] = boxCenter + static_cast<float>(sx) * ex +
        ...
        std::array<glm::vec3, 4> CollisionBody::faceCorners(int normalAxisIndex,
                                                            const glm::vec3& outwardNormal) const {
        ...
        void gatherCandidateTriangles(const SphereBVHNode* node,
            ...
            if (node->isLeaf()) {
                triangleIndices.insert(triangleIndices.end(), node->triangleIndices.begin(),
        ...
        std::optional<BoxFaceSelection> selectBoxContactAxisSATImpl(const CollisionBody& bodyA,
            ...
            const glm::vec3 centerDelta(glm::dot(centerDeltaWorld, bodyA.axis(0)),
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                const float radiusB = bodyB.halfExtent(0) * absRotation[i][0] +
                ...
                if (overlap < minOverlap) {
                    ...
                    bestAxis = BoxFaceSelection{
                        .referenceIsA = true,
                        .axisIndex = i,
                        .referenceOutwardNormal =
                            centerDelta[i] >= 0.0f ? bodyA.axis(i) : -bodyA.axis(i),
            ...
            for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 3; ++j) {
                    ...
                    const float distance =
                        std::abs(centerDelta[(i + 2) % 3] * rotation[(i + 1) % 3][j] -
        ...
        bool generateBoxBoxContacts(const CollisionBody& bodyA, const CollisionBody& bodyB,
            ...
            const glm::vec3 contactNormal =
                referenceIsA ? -referenceOutwardNormal : referenceOutwardNormal;
            const glm::vec3 referenceFaceCenter =
                referenceBody.boxCenter +
                referenceOutwardNormal * referenceBody.halfExtent(faceAxis->axisIndex);
            ...
            std::vector<glm::vec3> clippedPolygon(incidentFaceCorners.begin(),
                                                  incidentFaceCorners.end());
            ...
            for (const auto& [tangent, extent] : tangents) {
                clippedPolygon = clipPolygonAgainstPlane(
                    clippedPolygon, referenceFaceCenter + tangent * extent, tangent);
                clippedPolygon = clipPolygonAgainstPlane(
                    clippedPolygon, referenceFaceCenter - tangent * extent, -tangent);
            }
            ...
            if (contacts.size() > 4) {
                std::sort(
                    ...
                    [](const MeshContact& a, const MeshContact& b) { return a.depth > b.depth; });
                contacts.resize(4);
            }
        ...
        void generateBoxPlaneContacts(const CollisionBody& sampleBody,
            ...
            for (const glm::vec3& worldCorner : sampleCorners) {
                const float signedDistance =
                    glm::dot(worldCorner - targetBody.planePoint, targetBody.planeNormal);
                ...
                const glm::vec3 worldPointOnPlane =
                    worldCorner - signedDistance * targetBody.planeNormal;
                contacts.push_back({
                    &sampleBody,
                    &targetBody,
                    worldCorner,
        ...
        void generateVertexContacts(const CollisionBody& sampleBody,
                                    const CollisionBody& targetBody,
                                    std::vector<MeshContact>& contacts) {
            if (!sampleBody.bvhRoot || !targetBody.bvhRoot || !sampleBody.mesh ||
                !targetBody.mesh) {
                return;
            }
            ...
            for (const glm::vec3& worldVertex : sampleBody.sampleWorldVertices) {
                ...
                gatherCandidateTriangles(targetBody.bvhRoot, *targetBody.rigidBody, query,
                                         candidateTriangles);
                ...
                for (uint32_t triangleIndex : candidateTriangles) {
                    ...
                    bestPenetrationDepth =
                        std::max(0.0f, -glm::dot(worldVertex - closestPoint, geometricNormal));
                }
        ...
        void reduceContacts(std::vector<MeshContact>& contacts, float minPenetrationDepth) {
            ...
            contacts.erase(std::remove_if(contacts.begin(), contacts.end(),
                                          [minPenetrationDepth](const MeshContact& contact) {
            ...
            for (const auto& contact : contacts) {
                ...
                for (const auto& kept : reduced) {
                    if (glm::length2(contact.contactPoint - kept.contactPoint) <
                        kMinContactSpacingSq) {
                        tooClose = true;
                        break;
                    }
                }
        ...
        std::optional<CollisionBody> buildCollisionBody(uint32_t index,
                                                        scrap::RigidBodyComponent* rigidBody) {
            ...
            auto* meshComp = owner->getComponent<scrap::MeshRendererComponent>();
            ...
            for (const auto& vertex : vertices) {
                worldVertices.push_back(rigidBody->getPosition() +
                                        rigidBody->getOrientation() *
                                            scalePoint(vertex.position, bodyScale));
            }
            ...
            const glm::vec3 worldBoxCenter =
                boxShape
                    ? rigidBody->getPosition() +
                          rigidBody->getOrientation() * scalePoint(boxShape->localCenter, bodyScale)
                    : glm::vec3(0.0f);
            const glm::vec3 worldBoxHalfExtents =
                boxShape ? scalePoint(boxShape->localHalfExtents, scaleAbs) : glm::vec3(0.0f);
            const glm::vec3 worldPlanePoint =
                planeShape
                    ? rigidBody->getPosition() + rigidBody->getOrientation() *
                                                     scalePoint(planeShape->localPoint, bodyScale)
                    : glm::vec3(0.0f);
            const glm::vec3 worldPlaneNormal =
                planeShape ? glm::normalize(rigidBody->getOrientation() * planeShape->localNormal)
        ...
        struct CollisionDebugCounters {
            ...
            void print(size_t bodyCount, bool enabled) const {
                ...
                std::fprintf(
                    stderr,
                    "[collision] bodies=%zu noBvh=%d broadFail=%d narrowEmpty=%d depthFilter=%d "
                    "accepted=%d satSep=%d edgeEdge=%d faceEmpty=%d faceOk=%d\n",
                    bodyCount, noBvh, broadFail, narrowEmpty, depthFilter, accepted, boxSatSep,
                    boxEdgeEdge, boxFaceEmpty, boxFaceOk);
            }
        };
        ...
        class PairContactBuilder {
          ...
          private:
            static void appendGeneratedContacts(const CollisionBody& sampleBody,
                                                ...
                                                std::vector<MeshContact>& out, auto&& generate) {
            ...
            static void appendDirection(const CollisionBody& sampleBody,
                ...
                if (sampleBody.isBox && targetBody.isPlane) {
                    appendGeneratedContacts(sampleBody, targetBody, kContactDepthEpsilon, out,
                                            generateBoxPlaneContacts);
                    return;
                }
                appendGeneratedContacts(sampleBody, targetBody, 0.0005f, out,
                                        generateVertexContacts);
            }
            ...
            void appendBoxPair(const CollisionBody& bodyA, const CollisionBody& bodyB) {
                ...
                if (handledAnalytically) {
                    ...
                    ++debug.boxEdgeEdge;
                    appendGeneratedContacts(bodyA, bodyB, 0.0005f, boxContacts,
                                            generateVertexContacts);
                    appendGeneratedContacts(bodyB, bodyA, 0.0005f, boxContacts,
                                            generateVertexContacts);
                }
        ...
        SupportInfo buildSupportInfo(const CollisionBody& body, const glm::vec3& up) {
            ...
            if (body.isBox) {
                ...
                for (int axisIndex = 0; axisIndex < 3; ++axisIndex) {
                    verticalExtent +=
                        std::abs(glm::dot(body.axis(axisIndex), up)) * body.halfExtent(axisIndex);
                }
                ...
                info.horizontalRadius =
                    body.bvhRoot ? transformSphere(body.bvhRoot->sphere, *body.rigidBody).radius
            ...
            if (body.bvhRoot) {
                const SphereCollider sphere =
                    transformSphere(body.bvhRoot->sphere, *body.rigidBody);
        ...
        for (size_t i = 0; i < bodies.size(); ++i) {
            for (size_t j = i + 1; j < bodies.size(); ++j) {
                ...
                for (const auto& contact : contacts) {
                    ...
                    const float pairMargin =
                        contact.bodyA->contactMargin + contact.bodyB->contactMargin;
    ...
    std::vector<CollisionConstraint> XPBDSolver::generateCollisionConstraints(
        ...
        struct ConstraintBuilder {
            ...
            const std::map<std::pair<uintptr_t, uintptr_t>, std::vector<float>>&
                collisionLambdaWarmStart;
            const scrap::RigidBodyComponent* dragBody;
            ...
            ConstraintBuilder(const std::vector<scrap::RigidBodyComponent*>& rigidBodies,
                              const std::vector<bool>& wasActiveBeforeWake,
                              const std::map<std::pair<uintptr_t, uintptr_t>, std::vector<float>>&
                                  collisionLambdaWarmStart,
                              const scrap::RigidBodyComponent* dragBody,
                              float dragContactFrictionScale, size_t reserveCount)
                : rigidBodies(rigidBodies), wasActiveBeforeWake(wasActiveBeforeWake),
                  collisionLambdaWarmStart(collisionLambdaWarmStart), dragBody(dragBody),
                  dragContactFrictionScale(dragContactFrictionScale) {
                constraints.reserve(reserveCount);
            }

            static bool lessThan(const CollisionContact& a, const CollisionContact& b,
                                 const std::vector<scrap::RigidBodyComponent*>& rigidBodies) {
            ...
            bool shouldWake(uint32_t wakerIdx, scrap::RigidBodyComponent* sleeper) const {
                if (!sleeper->isSleeping() || !sleeper->canBeDynamic() ||
                    !wasActiveBeforeWake[wakerIdx]) {
                    return false;
                }
                ...
                return waker->isDynamic() ||
                       glm::length2(waker->getVelocity()) > kKinematicWakeSpeedSq;
            }

            float warmLambdaFor(scrap::RigidBodyComponent* bodyA,
                                scrap::RigidBodyComponent* bodyB) {
            ...
            float frictionFor(scrap::RigidBodyComponent* bodyA,
                              scrap::RigidBodyComponent* bodyB) const {
            ...
            void append(const CollisionContact& contact) {
                ...
                if (shouldWake(contact.indexA, bodyB)) {
                    bodyB->wake();
                }
                if (shouldWake(contact.indexB, bodyA)) {
                    bodyA->wake();
                }

                const glm::vec3 centerA = bodyA->getWorldCenterOfMass();
                const glm::vec3 centerB = bodyB->getWorldCenterOfMass();
                const glm::vec3 localOffsetA =
                    glm::inverse(bodyA->getOrientation()) * (contact.pointA - centerA);
                const glm::vec3 localOffsetB =
                    glm::inverse(bodyB->getOrientation()) * (contact.pointB - centerB);
                const glm::vec3 warmMid = 0.5f * (contact.pointA + contact.pointB);

                constraints.emplace_back(contact.indexA, contact.indexB, contact.contactNormal,
                                         localOffsetA, localOffsetB, 0.0f, warmMid,
                                         warmLambdaFor(bodyA, bodyB), frictionFor(bodyA, bodyB));
            }
        };
        ...
        ConstraintBuilder builder(rigidBodies, wasActiveBeforeWake, collisionLambdaWarmStart,
                                  dragBody, dragContactFrictionScale, contacts.size());
        for (const auto& contact : contacts) {
    ...
    void XPBDSolver::wakeUnsupportedBodies(
        ...
        while (anyWoken) {
            ...
            for (size_t i = 0; i < infos.size(); ++i) {
                ...
                if (!body->isSleeping() || !body->canBeDynamic())
                    continue;
                ...
                for (size_t j = 0; j < infos.size(); ++j) {
                    ...
                    if (supporter->canBeDynamic() && !supporter->isSleeping())
                        continue;

                    const float supportAlt = glm::dot(infos[j].center, up);
                    if (supportAlt >= sleepingAlt)
                        continue;

                    if (infos[j].ceilingAltitude < sleepingFloor - kSupportVerticalMargin)
                        continue;
