...
namespace scrap {
    namespace {
        ...
        std::shared_ptr<modeling::Material> makeWoodMaterial(
            const std::string& name, const glm::vec4& tint, float roughness, float metallic,
            ...
            material->setTexture(modeling::TextureType::BaseColor,
                                 std::make_shared<modeling::Texture>(
                                     (textureDirectory / "wood_table_001_diff_2k.png").string(),
                                     modeling::TextureType::BaseColor, true));
            material->setTexture(modeling::TextureType::Normal,
                                 std::make_shared<modeling::Texture>(
                                     (textureDirectory / "wood_table_001_nor_gl_2k.png").string(),
                                     modeling::TextureType::Normal, false));
            material->setTexture(modeling::TextureType::MetallicRoughness,
                                 std::make_shared<modeling::Texture>(
                                     (textureDirectory / "wood_table_001_rough_2k.png").string(),
                                     modeling::TextureType::MetallicRoughness, false));
        ...
        void rewriteWoodTextureUris(const std::filesystem::path& scenePath) {
            ...
            if (!input) {
                throw std::runtime_error("Failed to reopen scene for URI patch: " +
                                         scenePath.string());
            }
            ...
            if (!output) {
                throw std::runtime_error("Failed to write URI-patched scene: " +
                                         scenePath.string());
            }
            output << text;
        }

        void addMeshEntity(Scene& scene, const std::string& name,
            ...
            entity.addComponent<TransformComponent>(
                modeling::Transform(translation, rotation, scale));
...
int main() {
    ...
    try {
        std::filesystem::path projectRoot = scrap::platform::findProjectRootNearExecutable();
        if (projectRoot.empty()) {
            projectRoot = std::filesystem::canonical(std::filesystem::current_path());
        }
        const auto textureDirectory =
            std::filesystem::absolute(projectRoot / "testScene" / "textures2");
        ...
        const auto blockMaterial = makeWoodMaterial(
            "JengaBlockWood", glm::vec4(1.08f, 0.88f, 0.66f, 1.0f), 0.82f, 0.03f, textureDirectory);
        const auto tableMaterial = makeWoodMaterial(
            "JengaTableWood", glm::vec4(0.74f, 0.58f, 0.43f, 1.0f), 0.76f, 0.02f, textureDirectory);
        const auto tableMesh = makeTableMesh(8.0f, 8.0f, 6.0f);

        addMeshEntity(scene, "Table", tableMesh, tableMaterial, glm::vec3(0.0f, 0.0f, 0.0f),
        ...
        const glm::vec3 blockScale(kBlockLength * 0.5f, kBlockWidth * 0.5f, kBlockHeight * 0.5f);
        ...
        for (int layer = 0; layer < kLayerCount; ++layer) {
            ...
            const glm::quat rotation =
                alongX ? glm::quat(1.0f, 0.0f, 0.0f, 0.0f) : rotateQuarterTurn;
            const float z =
                blockHalfHeight + kTowerBaseClearance + static_cast<float>(layer) * layerStride;
            ...
            for (int slot = -1; slot <= 1; ++slot) {
                ...
                addMeshEntity(scene, "JengaBlock_" + std::to_string(blockIndex++), cubeMesh,
                              blockMaterial, position, rotation, blockScale);
            }
        }
