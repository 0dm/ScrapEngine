...
namespace scrap::launcher {
    ...
    namespace {
        ...
        std::string directoryGroup(const fs::path& rootPath, const fs::path& path,
        ...
        bool shouldSkipDirectory(const fs::path& path) {
            ...
            return name == "build" || name == "out" || name == "dist" ||
                   name == "vcpkg_installed" || name.starts_with("cmake-build");
        }
        ...
        bool looksLikeAuthoredScene(const fs::path& rootPath, const fs::path& path) {
            ...
            return containsAnyToken(normalizedPath,
                                    {"testscene", "/scene", "scene_", "_scene", "/scenes/"});
        }

        void appendDiscoveredMatches(AssetCatalog& catalog, const fs::path& rootPath,
            ...
            while (!ec && it != end) {
                ...
                if (entry.is_regular_file(ec)) {
                    ...
                    } else if (hasExtension(path, iblExtensions) &&
                               looksLikeEnvironmentMap(rootPath, path)) {
    ...
    AssetCatalog discoverAssetCatalog(const fs::path& rootPath) {
        ...
        static const std::set<std::string> iblExtensions = {".hdr", ".exr", ".jpg", ".jpeg",
                                                            ".png"};
    ...
    std::string buildCommandLinePreview(const AppOptions& options,
        ...
        if (!options.polyhaven_model_id.empty()) {
            ...
            if (!options.polyhaven_model_resolution.empty()) {
                stream << " --polyhaven-model-res "
                       << quoteIfNeeded(options.polyhaven_model_resolution);
            }
        }
        if (!options.polyhaven_hdri_id.empty()) {
            ...
            if (!options.polyhaven_hdri_resolution.empty()) {
                stream << " --polyhaven-hdri-res "
                       << quoteIfNeeded(options.polyhaven_hdri_resolution);
            }
        }
