...
namespace scrap::launcher {
    ...
    namespace {
        ...
        int findStringOptionIndex(const auto& options, const std::string& value,
                                  int fallbackIndex) {
        ...
        std::string formatBytes(uintmax_t bytes) {
            ...
            std::ostringstream stream;
            stream << std::fixed << std::setprecision(unitIndex == 0 ? 0 : 1) << value << ' '
                   << kUnits[unitIndex];
            return stream.str();
        }
        ...
        const PolyHavenAssetSummary* findAssetById(const std::vector<PolyHavenAssetSummary>& assets,
                                                   const std::string& id) {
            if (id.empty()) {
                return nullptr;
            }
            ...
            const auto it =
                std::find_if(assets.begin(), assets.end(),
                             [&](const PolyHavenAssetSummary& asset) { return asset.id == id; });
            return it == assets.end() ? nullptr : &(*it);
        }

    } // namespace

    AppLauncher::AppLauncher(std::filesystem::path assetRoot) : assetRoot(std::move(assetRoot)) {
    }

    void AppLauncher::initialize(uint32_t width, uint32_t height,
        ...
        polyHavenModelResolution =
            initialPolyHavenModelResolution.empty() ? "2k" : initialPolyHavenModelResolution;
        polyHavenHdriId = initialPolyHavenHdriId;
        polyHavenHdriResolution =
            initialPolyHavenHdriResolution.empty() ? "4k" : initialPolyHavenHdriResolution;
        ...
        state.scenePath =
            initialScenePath.empty()
                ? (state.catalog.launchTargets.empty() ? std::string()
                                                       : state.catalog.launchTargets.front().path)
                : initialScenePath;
        state.iblPath = initialIblPath.empty()
                            ? (state.catalog.iblMaps.empty() ? std::string()
                                                             : state.catalog.iblMaps.front().path)
                            : initialIblPath;
    ...
    void AppLauncher::pollBackgroundTasks(
        ...
        auto pollCatalog = [&](RemoteBrowserState& browserState) {
            ...
            browserState.statusMessage =
                "Loaded " + std::to_string(browserState.assets.size()) + " assets from Poly Haven.";
        };
    ...
    std::array<float, 3> AppLauncher::suggestedModelRotationDegrees() const {
        ...
        return pathLooksLikeAuthoredScene(state.scenePath) ? std::array<float, 3>{0.0f, 0.0f, 0.0f}
                                                           : defaultModelRotationDegrees();
    }
    ...
    void AppLauncher::render(const std::function<bool(const LaunchRequest&)>& finalizeLaunch,
                             const std::function<void()>& requestQuit) {
        ImGuiViewport* viewport = ImGui::GetMainViewport();
        ImDrawList* drawList = ImGui::GetBackgroundDrawList(viewport);
        const ImVec2 min = viewport->WorkPos;
        const ImVec2 max = ImVec2(viewport->WorkPos.x + viewport->WorkSize.x,
        ...
        drawList->AddRectFilledMultiColor(min, max, theme.bgTop, theme.bgTop, theme.bgBottom,
                                          theme.bgBottom);
        for (float x = min.x - 32.0f; x < max.x + 32.0f; x += 48.0f) {
            drawList->AddLine(ImVec2(x, min.y), ImVec2(x + 120.0f, max.y), theme.bgGrid, 1.0f);
        }
        drawList->AddRectFilled(ImVec2(max.x - 260.0f, min.y), ImVec2(max.x, min.y + 6.0f),
                                theme.accent);
        drawList->AddRectFilled(ImVec2(min.x, max.y - 4.0f), ImVec2(min.x + 220.0f, max.y),
        ...
        ImGui::Begin("ScrapLauncher", nullptr, flags);
        ...
        auto beginSurface = [&](const char* id, const ImVec2& size,
                                const ImVec2& padding = ImVec2(18.0f, 16.0f)) {
        ...
        const bool selectedModelCached =
            !polyHavenModelId.empty() &&
            sortedContains(state.cache.summary.cachedModelIds, polyHavenModelId);
        const bool selectedHdriCached =
            !polyHavenHdriId.empty() &&
        ...
        auto applyCacheMutation = [&](const PolyHavenCacheMutationResult& result,
                                      const char* emptyMessage, const char* successPrefix) {
            if (!result.errorMessage.empty()) {
        ...
        ImGui::TextColored(theme.muted, "ScrapEngine");
        ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 2.0f);
        ImGui::SetWindowFontScale(1.35f);
        ImGui::TextUnformatted("Launcher");
        ImGui::SetWindowFontScale(1.0f);
        ImGui::TextColored(ImVec4(0.74f, 0.84f, 0.95f, 1.0f),
                           "%zu local targets  |  %zu local maps  |  %u x %u  |  %s cache",
                           state.catalog.launchTargets.size(), state.catalog.iblMaps.size(),
                           state.launchWidth, state.launchHeight,
        ...
        const float contentHeight =
            std::max(360.0f, ImGui::GetContentRegionAvail().y - footerHeight - theme.gap);
        const bool stackedLayout = fullWidth < 1180.0f;
        const float sidebarWidth =
            stackedLayout ? fullWidth : std::clamp(fullWidth * 0.34f, 360.0f, 460.0f);
        const float browserWidth =
            stackedLayout ? fullWidth : std::max(420.0f, fullWidth - sidebarWidth - theme.gap);
        const float browserHeight =
            stackedLayout ? std::max(240.0f, (contentHeight - (theme.gap * 2.0f)) / 3.0f)
                          : std::max(240.0f, (contentHeight - theme.gap) * 0.5f);
        const float sidebarHeight =
            stackedLayout
        ...
        auto startRemoteLoadIfNeeded = [&](RemoteBrowserState& browserState,
                                           PolyHavenAssetType type) {
            if (!browserState.assets.empty() || browserState.pendingLoad.has_value() ||
                !browserState.statusMessage.empty()) {
                return;
            }

            browserState.statusMessage = kPolyHavenLoadingMessage;
            browserState.pendingLoad =
                std::async(std::launch::async, [type]() { return fetchPolyHavenAssets(type); });
        };

        auto renderRemoteAssetList =
            [&](RemoteBrowserState& browserState, const auto& resolutionOptions,
                int& resolutionIndex, std::string& selectedId, std::string& selectedResolution,
                std::string& localPathToClear, int& selectedLocalIndex, const char* searchLabel,
                const char* refreshLabel, const char* presetLabel, PolyHavenAssetType assetType,
                const char* listId, const std::vector<std::string>& cachedIds) {
                ...
                ImGui::InputTextWithHint(searchLabel, "Filter by name, tag, or category",
                                         &browserState.searchQuery);
                ImGui::SameLine();
                ImGui::SetNextItemWidth(140.0f);
                const std::string sortLabel = std::string("Sort##") + listId;
                ImGui::Combo(sortLabel.c_str(), &browserState.selectedSortMode,
                             kRemoteSortOptions.data(),
                             static_cast<int>(kRemoteSortOptions.size()));

                const int safeResolutionIndex =
                    std::clamp(resolutionIndex, 0, static_cast<int>(resolutionOptions.size()) - 1);
                const char* resolutionLabel =
                    resolutionOptions[static_cast<size_t>(safeResolutionIndex)].label;
                ...
                if (ImGui::BeginCombo(presetLabel, resolutionLabel)) {
                    for (int i = 0; i < static_cast<int>(resolutionOptions.size()); ++i) {
                        ...
                        if (ImGui::Selectable(resolutionOptions[static_cast<size_t>(i)].label,
                                              selected)) {
                            resolutionIndex = i;
                ...
                for (int i = 0; i < static_cast<int>(browserState.assets.size()); ++i) {
                    if (matchesRemoteSearch(browserState.assets[static_cast<size_t>(i)],
                                            browserState.searchQuery)) {
                ...
                for (const int i : visibleIndices) {
                    ...
                    if (ImGui::Selectable((rowLabel.str() + "##" + asset.id).c_str(), selected)) {
                        ...
                        selectedId = asset.id;
                        selectedResolution =
                            resolutionOptions[static_cast<size_t>(safeResolutionIndex)].value;
        ...
        if (ImGui::BeginTabBar("SceneSourceTabs")) {
            if (ImGui::BeginTabItem(kLocalSceneTabLabel)) {
                ...
                ImGui::InputTextWithHint("##manualScene", "Custom glTF / GLB path",
                                         &state.scenePath);
                ...
                for (int i = 0; i < static_cast<int>(state.catalog.launchTargets.size()); ++i) {
                    ...
                    const bool selected =
                        state.selectedLaunchTarget == i && polyHavenModelId.empty();
            ...
            if (ImGui::BeginTabItem(kRemoteSceneTabLabel)) {
                ...
                renderRemoteAssetList(state.remoteModels, kModelResolutionOptions,
                                      state.remoteModels.selectedResolutionIndex, polyHavenModelId,
                                      polyHavenModelResolution, state.scenePath,
                                      state.selectedLaunchTarget, "##polyhaven-model-search",
                                      "Refresh Models", "Download preset##polyhaven-model-preset",
                                      PolyHavenAssetType::Model, "PolyHavenModelList",
        ...
        if (ImGui::BeginTabBar("IblSourceTabs")) {
            if (ImGui::BeginTabItem(kLocalSceneTabLabel)) {
                ...
                ImGui::InputTextWithHint("##manualIbl", "Custom HDR / environment map path",
                                         &state.iblPath);
                if (ImGui::IsItemEdited()) {
            ...
            if (ImGui::BeginTabItem(kRemoteSceneTabLabel)) {
                ...
                renderRemoteAssetList(state.remoteHdris, kHdriResolutionOptions,
                                      state.remoteHdris.selectedResolutionIndex, polyHavenHdriId,
                                      polyHavenHdriResolution, state.iblPath, state.selectedIblMap,
                                      "##polyhaven-hdri-search", "Refresh HDRIs",
                                      "Download preset##polyhaven-hdri-preset",
                                      PolyHavenAssetType::Hdri, "PolyHavenHdriList",
        ...
        if (polyHavenModelId.empty()) {
            ImGui::TextWrapped(
                "%s", describeLocalSelection(state.scenePath, kDefaultLauncherScene).c_str());
        } else if (selectedSceneAsset != nullptr) {
            ImGui::TextWrapped("%s", selectedSceneAsset->name.c_str());
            ImGui::TextDisabled("Poly Haven  |  %s  |  %s%s", polyHavenModelId.c_str(),
        ...
        if (polyHavenHdriId.empty()) {
            ImGui::TextWrapped("%s", state.iblPath.empty() ? "Engine default lighting"
                                                           : state.iblPath.c_str());
        } else if (selectedHdriAsset != nullptr) {
            ImGui::TextWrapped("%s", selectedHdriAsset->name.c_str());
            ImGui::TextDisabled("Poly Haven  |  %s  |  %s%s", polyHavenHdriId.c_str(),
        ...
        float rotationDegrees[3] = {state.modelRotationDegrees[0], state.modelRotationDegrees[1],
                                    state.modelRotationDegrees[2]};
        ImGui::SetNextItemWidth(-1.0f);
        if (ImGui::InputFloat3("##model-rotation", rotationDegrees, "%.0f deg")) {
            state.modelRotationDegrees = {rotationDegrees[0], rotationDegrees[1],
                                          rotationDegrees[2]};
        ...
        const char* resolutionLabel =
            state.selectedResolutionPreset >= 0
                ? kLauncherResolutionOptions[static_cast<size_t>(state.selectedResolutionPreset)]
                      .label
                : "Custom";
        ...
        if (focusedRemoteAsset != nullptr) {
            ...
            ImGui::TextDisabled("%s  |  %s  |  %d downloads", focusedRemoteAsset->id.c_str(),
                                focusedRemoteAsset->maxResolutionLabel.empty()
                                    ? "max res ?"
                                    : focusedRemoteAsset->maxResolutionLabel.c_str(),
                                focusedRemoteAsset->downloadCount);
            if (!focusedRemoteAsset->categories.empty()) {
                ImGui::Dummy(ImVec2(0.0f, 4.0f));
                ImGui::TextDisabled("Categories: %s",
                                    joinStrings(focusedRemoteAsset->categories, ", ").c_str());
            }
            if (!focusedRemoteAsset->tags.empty()) {
                ImGui::TextDisabled("Tags: %s",
                                    joinStrings(focusedRemoteAsset->tags, ", ").c_str());
            }
            if (!focusedRemoteAsset->description.empty()) {
                ImGui::Dummy(ImVec2(0.0f, 8.0f));
                ImGui::BeginChild("RemoteAssetDescription", ImVec2(0.0f, 94.0f),
        ...
        ImGui::TextDisabled("%zu models  |  %zu HDRIs  |  %zu files",
                            state.cache.summary.modelAssetCount, state.cache.summary.hdriAssetCount,
        ...
        if (ImGui::Button("Prune stale")) {
            applyCacheMutation(
                ...
                "No stale cached assets found.", "Pruned stale assets.");
        }
        if (ImGui::Button("Clear cache")) {
            applyCacheMutation(clearPolyHavenCache(), "Cache was already empty.",
        ...
        if (selectedModelCached) {
            if (ImGui::Button("Remove model")) {
                applyCacheMutation(
                    ...
                    "Selected model was not cached.", "Removed cached model.");
            }
        ...
        if (selectedHdriCached) {
            ...
            if (ImGui::Button("Remove HDRI")) {
                applyCacheMutation(
                    ...
                    "Selected HDRI was not cached.", "Removed cached HDRI.");
            }
        }
        ...
        ImGui::InputTextMultiline("##command-preview", &state.commandPreview, ImVec2(-1.0f, 96.0f),
        ...
        ImGui::BeginGroup();
        ImGui::TextColored(launchBusy ? ImGui::ColorConvertU32ToFloat4(theme.accent) : theme.subtle,
                           "%s",
                           launchBusy ? "Downloading selected remote assets into the local cache."
    ...
    void AppLauncher::updatePreview() {
        ...
        if (polyHavenModelId.empty()) {
            ...
            const auto modelCachePath =
                getPolyHavenModelCachePath(polyHavenModelId, polyHavenModelResolution);
        ...
        if (polyHavenHdriId.empty()) {
            ...
            const auto hdriCachePath =
                getPolyHavenHdriCachePath(polyHavenHdriId, polyHavenHdriResolution);
    ...
    bool AppLauncher::launchFromUi(
        ...
        if (!polyHavenModelId.empty() || !polyHavenHdriId.empty()) {
            ...
            state.pendingLaunch = std::async(std::launch::async, [modelId, modelResolution, hdriId,
                                                                  hdriResolution, localScenePath,
                                                                  localIblPath]() {
                ...
                if (!modelId.empty()) {
                    ...
                    if (!modelDownload.errorMessage.empty()) {
                        result.errorMessage =
                            "Poly Haven model download failed: " + modelDownload.errorMessage;
                        return result;
                    }
                ...
                if (!hdriId.empty()) {
                    ...
                    if (!hdriDownload.errorMessage.empty()) {
                        result.errorMessage =
                            "Poly Haven HDRI download failed: " + hdriDownload.errorMessage;
                        return result;
                    }
    ...
    bool AppLauncher::finalizeLaunchRequest(
        const std::function<bool(const LaunchRequest&)>& finalizeLaunch,
        const std::string& resolvedScenePath, const std::string& resolvedIblPath) {
        ...
        if (!finalizeLaunch(request)) {
            state.statusMessage =
                "Failed to load the selected scene. Check the console for loader errors.";
            return false;
        }
