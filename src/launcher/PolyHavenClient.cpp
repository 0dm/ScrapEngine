...
namespace scrap::launcher {
    ...
    namespace {
        ...
        constexpr const char* kUserAgent =
            "ScrapEngineLauncher/0.1 (+https://github.com/aaron/ScrapEngine)";
        ...
        std::string pickBestResolution(const json& filesNode,
        ...
        PolyHavenAssetSummary parseAssetSummary(const std::string& id, const json& node) {
            ...
            summary.maxResolutionLabel =
                formatMaxResolutionLabel(node.value("max_resolution", json::array()));
            return summary;
        }

        std::vector<RemoteFile> hdriFileFromInfo(const json& filesNode, const std::string& id,
                                                 const std::string& preferredResolution) {
            ...
            throw std::runtime_error("No .hdr download is available for " + id + " at " +
                                     resolution);
        }

        std::vector<RemoteFile> modelFilesFromInfo(const json& filesNode, const std::string& id,
                                                   const std::string& preferredResolution) {
        ...
        void collectCachedAssetIds(const fs::path& root, std::vector<std::string>& ids,
                                   size_t& assetCount) {
            std::error_code error;
            if (!fs::exists(root, error)) {
                return;
        ...
        PolyHavenDownloadResult downloadAssetFiles(const std::string& id,
            ...
            try {
                const json filesNode =
                    json::parse(httpGetText(std::string(kApiBaseUrl) + "/files/" + id));
                const std::vector<RemoteFile> files =
                    filesBuilder(filesNode, id, preferredResolution);
                if (files.empty()) {
    ...
    std::filesystem::path getPolyHavenCacheRoot() {
        return userHomePath() / "Library" / "Caches" / "ScrapEngine" / "polyhaven";
    }

    std::filesystem::path getPolyHavenHdriCachePath(const std::string& id,
                                                    const std::string& resolutionLabel) {
        return getPolyHavenCacheRoot() / "hdris" / id / resolutionLabel /
               (id + "_" + resolutionLabel + ".hdr");
    }

    std::filesystem::path getPolyHavenModelCachePath(const std::string& id,
                                                     const std::string& resolutionLabel) {
        return getPolyHavenCacheRoot() / "models" / id / resolutionLabel /
               (id + "_" + resolutionLabel + ".gltf");
    }

    PolyHavenAssetListResult fetchPolyHavenAssets(PolyHavenAssetType type) {
        try {
            const std::string url =
                std::string(kApiBaseUrl) + "/assets?type=" + assetTypeQueryValue(type);
            ...
            std::ranges::sort(result.assets,
                              [](const PolyHavenAssetSummary& lhs,
                                 const PolyHavenAssetSummary& rhs) { return lhs.name < rhs.name; });
    ...
    PolyHavenDownloadResult downloadPolyHavenHdri(const std::string& id,
                                                  const std::string& resolutionLabel) {
    ...
    PolyHavenDownloadResult downloadPolyHavenModelGltf(const std::string& id,
                                                       const std::string& resolutionLabel) {
    ...
    PolyHavenCacheSummary inspectPolyHavenCache() {
        ...
        collectCachedAssetIds(cacheTypeRoot(PolyHavenAssetType::Model), summary.cachedModelIds,
                              summary.modelAssetCount);
        collectCachedAssetIds(cacheTypeRoot(PolyHavenAssetType::Hdri), summary.cachedHdriIds,
                              summary.hdriAssetCount);
        return summary;
    }

    PolyHavenCacheMutationResult removePolyHavenCachedAsset(PolyHavenAssetType type,
                                                            const std::string& id) {
    ...
    PolyHavenCacheMutationResult prunePolyHavenCacheOlderThan(std::chrono::hours maxAge) {
        ...
        for (const PolyHavenAssetType type :
             {PolyHavenAssetType::Model, PolyHavenAssetType::Hdri}) {
