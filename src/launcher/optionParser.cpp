...
AppOptions::AppOptions(int argc, char const** argv) : desc("Allowed options") {
    ...
    desc.add_options()("help", "produce help message")(
        ...
        "post-import model rotation around world Y in degrees")(
        "model-rotate-z", po::value<double>(&(this->model_rotate_z_degrees)),
        "post-import model rotation around world Z in degrees")(
        "model-yaw", po::value<double>(),
        "legacy alias for a single-axis post-import model rotation in degrees")(
        "model-axis", po::value<std::string>(),
        "legacy axis selector used with --model-yaw (x, y, z)")(
        "input-file,f", po::value<std::string>(&(this->scene_file))->default_value(""),
        "scene file to load")("camera_collide", po::bool_switch(&(this->camera_collide)),
                              "enable camera collision/pushing against dynamic objects")(
        "ibl,i", po::value<std::string>(&(this->ibl_file))->default_value(""),
        "HDR IBL map to load")(
        "polyhaven-model", po::value<std::string>(&(this->polyhaven_model_id))->default_value(""),
        "Poly Haven model ID to cache and load")(
        "polyhaven-model-res",
        po::value<std::string>(&(this->polyhaven_model_resolution))->default_value("2k"),
        "Poly Haven model resolution preset (1k, 2k, 4k)")(
        "polyhaven-hdri", po::value<std::string>(&(this->polyhaven_hdri_id))->default_value(""),
        "Poly Haven HDRI ID to cache and load")(
        "polyhaven-hdri-res",
        po::value<std::string>(&(this->polyhaven_hdri_resolution))->default_value("4k"),
        "Poly Haven HDRI resolution preset (1k, 2k, 4k, 8k, 16k)");
    ...
    const bool hasExplicitRotation = vm.count("model-rotate-x") > 0 ||
