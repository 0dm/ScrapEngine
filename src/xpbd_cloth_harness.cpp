...
namespace {
    ...
    bool approxEqual(const glm::vec3& actual, const glm::vec3& expected,
                     float epsilon = kPositionEpsilon) {
        return glm::length(actual - expected) <= epsilon;
    }

    bool approxEqual(const glm::vec4& actual, const glm::vec4& expected,
                     float epsilon = kPositionEpsilon) {
        return glm::length(actual - expected) <= epsilon;
    }
    ...
    ClothParticle makeParticle(const glm::vec3& position, float invMass = 1.0f, bool pinned = false,
    ...
    scrap::Vertex makeRenderVertex(const glm::vec3& position, const glm::vec2& uv) {
        return scrap::Vertex{
            .position = position,
    ...
    std::shared_ptr<scrap::modeling::Mesh> makeQuadMesh() {
        ...
        auto mesh = std::make_shared<scrap::modeling::Mesh>(vertices, indices);
        mesh->generateNormals();
        mesh->generateTangents();
        return mesh;
    }

    scrap::ClothSettings makeClothSettings(int solverSubsteps = 4, float stretchCompliance = 0.0f,
                                           float bendCompliance = 0.0f, float damping = 0.0f,
    ...
    bool testSubstepsAffectIntegration(std::vector<std::string>& errors) {
        ...
        const scrap::ClothSettings oneStepSettings = makeClothSettings(1);
        oneStepSolver.solveCloth(oneSubstep, oneStepSettings, 1.0f, glm::vec3(0.0f, -10.0f, 0.0f));

        if (!approxEqual(oneSubstep.particles[0].position.y, -10.0f)) {
            appendError(
                errors,
                "substeps=1 integration did not match expected semi-implicit Euler position");
            return false;
        }
        ...
        const scrap::ClothSettings fourStepSettings = makeClothSettings(4);
        fourStepSolver.solveCloth(fourSubsteps, fourStepSettings, 1.0f,
                                  glm::vec3(0.0f, -10.0f, 0.0f));

        if (!approxEqual(fourSubsteps.particles[0].position.y, -6.25f)) {
            appendError(errors,
                        "substeps=4 integration did not produce the expected substepped position");
            return false;
        }

        if (!approxEqual(fourSubsteps.particles[0].velocity.y, -10.0f)) {
            appendError(errors,
                        "substeps=4 integration did not preserve the expected final velocity");
            return false;
        }
    ...
    bool testPinnedParticlesRemainFixed(std::vector<std::string>& errors) {
        ...
        const scrap::ClothSettings settings = makeClothSettings(4);
    ...
    bool testStretchConstraintRespectsPinningAndInvMass(std::vector<std::string>& errors) {
        ...
        XPBDSolver solver;
        solver.solverIterations = 8;
        const scrap::ClothSettings settings = makeClothSettings(1, 0.0f);
        ...
        if (!approxEqual(cloth.particles[1].position, glm::vec3(1.0f, 0.0f, 0.0f))) {
            appendError(errors, "stretch projection did not move the unpinned particle according "
                                "to inverse-mass weighting");
            return false;
        }
        return true;
    }
    ...
    bool testStretchComplianceSoftensProjection(std::vector<std::string>& errors) {
        ...
        const scrap::ClothSettings rigidSettings = makeClothSettings(1, 0.0f);
        const scrap::ClothSettings softSettings = makeClothSettings(1, 10.0f);
        solver.solveCloth(rigidCloth, rigidSettings, 1.0f, glm::vec3(0.0f));
        solver.solveCloth(softCloth, softSettings, 1.0f, glm::vec3(0.0f));

        const float rigidDistance =
            glm::length(rigidCloth.particles[1].position - rigidCloth.particles[0].position);
        const float softDistance =
            glm::length(softCloth.particles[1].position - softCloth.particles[0].position);
    ...
    bool testDampingAndGravityScaleAffectMotion(std::vector<std::string>& errors) {
        ...
        const scrap::ClothSettings dampingSettings = makeClothSettings(1, 0.0f, 0.0f, 0.5f, 0.0f);
        solver.solveCloth(dampedCloth, dampingSettings, 1.0f, glm::vec3(0.0f, -10.0f, 0.0f));

        if (!approxEqual(dampedCloth.particles[0].position.y, 5.0f) ||
            !approxEqual(dampedCloth.particles[0].velocity.y, 5.0f)) {
            appendError(errors,
                        "damping or gravityScale=0 did not affect particle motion correctly");
            return false;
        }
        ...
        const scrap::ClothSettings gravitySettings = makeClothSettings(1, 0.0f, 0.0f, 0.0f, 0.25f);
        solver.solveCloth(scaledGravityCloth, gravitySettings, 1.0f, glm::vec3(0.0f, -8.0f, 0.0f));
    ...
    bool testBendConstraintReducesError(std::vector<std::string>& errors) {
        ...
        cloth.bendConstraints.emplace_back(0, 1, 2, 0, 3, 0.0f, 0, 1, 0.0f);
        ...
        const scrap::ClothSettings settings = makeClothSettings(4, 0.0f, 0.0f);
        ...
        if (!(std::fabs(zAfter) < std::fabs(zBefore))) {
            appendError(
                errors,
                "bend projection did not move the free particle back toward the rest plane");
            return false;
        }
    ...
    bool testClothComponentRebuildFromMesh(std::vector<std::string>& errors) {
        ...
        scrap::Entity entity("ClothEntity");
        entity.addComponent<scrap::TransformComponent>(scrap::modeling::Transform(
            glm::vec3(10.0f, 2.0f, -1.0f),
            glm::angleAxis(1.57079632679f, glm::vec3(0.0f, 0.0f, 1.0f)), glm::vec3(1.0f)));
        entity.addComponent<scrap::ClothComponent>(mesh, settings);

        auto* clothComponent = entity.getComponent<scrap::ClothComponent>();
        ...
        if (!clothData || clothData->particles.size() != 4) {
            appendError(errors,
                        "ClothComponent cloth particle count did not match source mesh vertices");
            return false;
        }
        ...
        if (!approxEqual(clothData->particles[1].position, expectedWorldPosition)) {
            appendError(errors,
                        "ClothComponent did not initialize particle positions in world space");
            return false;
        }
        ...
        for (const auto& stretchConstraint : clothData->stretchConstraints) {
            if (!approxEqual(stretchConstraint.compliance, settings.stretchCompliance)) {
                appendError(
                    errors,
                    "ClothComponent did not apply stretch compliance to generated constraints");
                return false;
            }
        }
        ...
        for (const auto& bendConstraint : clothData->bendConstraints) {
            if (!approxEqual(bendConstraint.compliance, settings.bendCompliance)) {
                appendError(
                    errors,
                    "ClothComponent did not apply bend compliance to generated constraints");
                return false;
            }
        }

        scrap::ClothComponent invalidCloth;
    ...
    bool testClothComponentTransformSync(std::vector<std::string>& errors) {
        ...
        scrap::Entity entity("TransformSyncCloth");
        entity.addComponent<scrap::TransformComponent>(scrap::modeling::Transform(
            glm::vec3(2.0f, 0.0f, 0.0f), glm::quat(1.0f, 0.0f, 0.0f, 0.0f), glm::vec3(1.0f)));
        entity.addComponent<scrap::ClothComponent>(mesh);

        auto* clothComponent = entity.getComponent<scrap::ClothComponent>();
        auto* transformComponent = entity.getComponent<scrap::TransformComponent>();
        auto* clothData = clothComponent ? clothComponent->getClothData() : nullptr;
        if (!clothComponent || !transformComponent || !clothData ||
            clothData->particles.size() < 2) {
        ...
        if (!approxEqual(clothData->particles[1].position, glm::vec3(5.0f, 2.0f, 0.0f)) ||
            ...
            appendError(
                errors,
                "ClothComponent did not rigidly sync particle state to owner transform changes");
            return false;
        }
        ...
        scrap::Entity noTransformEntity("NoTransformCloth");
        noTransformEntity.addComponent<scrap::ClothComponent>(mesh);
        auto* noTransformCloth = noTransformEntity.getComponent<scrap::ClothComponent>();
        auto* noTransformClothData = noTransformCloth ? noTransformCloth->getClothData() : nullptr;
        if (!noTransformCloth || !noTransformClothData ||
            !approxEqual(noTransformClothData->particles[1].position,
                         glm::vec3(1.0f, 0.0f, 0.0f))) {
            appendError(errors,
                        "ClothComponent did not treat a missing TransformComponent as identity");
            return false;
        }
    ...
    bool testClothComponentRuntimeMeshSync(std::vector<std::string>& errors) {
        ...
        scrap::Entity entity("RuntimeMeshCloth");
        entity.addComponent<scrap::TransformComponent>(scrap::modeling::Transform(
        ...
        if (!runtimeMesh ||
            ...
            appendError(errors, "ClothComponent syncRuntimeMesh did not write inverse-transformed "
        ...
        for (const auto& vertex : runtimeMesh->getVertices()) {
            if (!isFinite(vertex.position) || !isFinite(vertex.normal) ||
                ...
                appendError(errors,
                            "ClothComponent syncRuntimeMesh produced non-finite mesh attributes");
                return false;
            }
        }
    ...
    bool testClothComponentRuntimeMeshTangentSyncModes(std::vector<std::string>& errors) {
        ...
        scrap::Entity entity("RuntimeMeshClothTangents");
        entity.addComponent<scrap::ClothComponent>(mesh);
        ...
        if (!approxEqual(runtimeMesh->getVertices()[0].tangent, sentinelTangent)) {
            appendError(errors,
                        "ClothComponent syncRuntimeMesh(false) unexpectedly regenerated tangents");
            return false;
        }
        ...
        for (const auto& vertex : runtimeMesh->getVertices()) {
            if (!isFinite(vertex.normal) || !isFinite(glm::vec3(vertex.tangent))) {
                appendError(errors,
                            "ClothComponent tangent sync modes produced non-finite basis data");
                return false;
            }
        }
