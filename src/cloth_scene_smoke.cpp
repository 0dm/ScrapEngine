...
namespace {
    ...
    std::filesystem::path writeClothFixture(const std::filesystem::path& fixtureStem,
                                            bool twoPrimitives) {
        const std::filesystem::path binPath = fixtureStem;
        const std::filesystem::path gltfPath =
            fixtureStem.parent_path() / (fixtureStem.stem().string() + ".gltf");
...

        {
            ...
            binOut.write(reinterpret_cast<const char*>(positions.data()),
                         static_cast<std::streamsize>(positions.size() * sizeof(float)));
            binOut.write(reinterpret_cast<const char*>(indices.data()),
                         static_cast<std::streamsize>(indices.size() * sizeof(uint32_t)));
        }

        const std::string extraPrimitive = twoPrimitives ? R"(,
          {
        ...
        const std::string nodeName = twoPrimitives ? "MultiClothNode" : "SingleClothNode";
        ...
        gltfOut
            ...
            << "  ],\n"
            << "  \"buffers\": [\n"
            << "    { \"uri\": \"" << binPath.filename().string() << "\", \"byteLength\": 48 }\n"
            << "  ],\n"
            << "  \"bufferViews\": [\n"
    ...
    bool testSinglePrimitiveClothImport(std::vector<std::string>& errors) {
        const auto fixtureStem =
            std::filesystem::temp_directory_path() / "scrap_single_cloth_fixture.bin";
        const std::filesystem::path gltfPath = writeClothFixture(fixtureStem, false);

        scrap::Scene scene({.scrWidth = 640, .scrHeight = 480});
        ...
        auto* meshRenderer = entity->getComponent<scrap::MeshRendererComponent>();
        if (!clothComponent || !clothComponent->hasClothData() || !meshRenderer) {
            errors.push_back(
                "single-primitive cloth scene did not create cloth + mesh renderer components");
            return false;
        }
        ...
        if (!clothComponent->getRuntimeMesh() ||
            ...
            errors.push_back(
                "single-primitive cloth scene did not swap the renderer to runtimeMesh");
            return false;
        }
        ...
        if (settings.solverSubsteps != 2 || settings.pinnedParticleIndices.size() != 1 ||
            ...
            errors.push_back(
                "single-primitive cloth scene did not preserve imported cloth settings");
            return false;
        }
    ...
    bool testMultiPrimitiveClothSkipped(std::vector<std::string>& errors) {
        const auto fixtureStem =
            std::filesystem::temp_directory_path() / "scrap_multi_cloth_fixture.bin";
        const std::filesystem::path gltfPath = writeClothFixture(fixtureStem, true);
        ...
        if (entity->getComponent<scrap::ClothComponent>() != nullptr) {
            errors.push_back("multi-primitive cloth scene should skip cloth component creation");
            return false;
        }

        if (entity->getComponents<scrap::MeshRendererComponent>().size() != 2) {
...
int main() {
    ...
    std::cout << "  single primitive cloth: " << (singlePrimitiveOk ? "ok" : "failed") << "\n";
    std::cout << "  multi primitive skip: " << (multiPrimitiveOk ? "ok" : "failed") << "\n";
    return 0;
}
